...
#include "mm/heap/alloc/block.h"
#include "mm/heap/alloc/freelist.h"
#include "mm/heap/alloc/large-blk.h"
...
void mman_deinit(mman_t man) {
    if (man == NULL)
        return;
    if (man->cb_delmem) {
        for (mman_pool_t p = man->main.next, n = p->next; p;
             p = n, n = p->next) {
...
size_t mman_alloced_size(mman_t man) { return man ? man->alloced_size : 0; }

size_t mman_total_size(mman_t man) { return man ? man->size : 0; }
...
void *mman_alloc(mman_t man, size_t size) {
    ...
    if (size >= ALLOC_LARGE_BLK_SIZE) {
        return large_blk_alloc(size, man->large, man->cb_reqmem,
                               man->cb_delmem);
    }

    // 优先从空闲链表中分配
    void *ptr = freelists_match(man->freed, size)
                    ?: freelist_match(&man->large_blk, size);

    if (ptr == NULL) { // 不足就分配
        if (!mman_reqmem(man, size))
            return NULL;
...
void *mman_aligned_alloc(mman_t man, size_t size, size_t align) {
    ...
    if (align < 2 * sizeof(size_t))
        return mman_alloc(man, size); // 对齐小于 2 倍指针大小
    size = size == 0 ? 2 * sizeof(size_t)
                     : PADDING(size); // 保证最小分配 2 个字长且对齐到 2 倍字长

    if (size >= ALLOC_LARGE_BLK_SIZE || align >= ALLOC_LARGE_BLK_SIZE) {
        void *ptr =
            large_blk_alloc(size, man->large, man->cb_reqmem, man->cb_delmem);
...
void mman_free(mman_t man, void *ptr) {
    ...
    if ((size_t)ptr % SIZE_4k == 0) {
        if (large_blk_free(man->large, ptr, man->cb_delmem))
            return;
    }
...
void *mman_aligned_realloc(mman_t man, void *ptr, size_t newsize,
                           size_t align) {
    if (ptr == NULL)
        return mman_aligned_alloc(man, newsize, align);
    if (align & (align - 1))
        return NULL; // 不是 2 的幂次方
    if (align < 2 * sizeof(size_t))
        return mman_realloc(man, ptr, newsize); // 对齐小于 2 倍指针大小
