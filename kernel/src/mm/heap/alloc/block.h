...
#define AREA_FLAG                                                              \
    ((size_t)2) // 分配区头尾标记 (防止块查找越界) (禁止为块设置该标记)
#define SIZE_FLAG                                                              \
    ((size_t)4)               // 2M 分配区标记 (仅对多分配区有效) (为 2M
                              // 分配区内所有块设置该标记)
...
#define blk_nexthead(ptr, size)                                                \
    (((size_t *)(ptr + size))[1]) // 下一个块的头部标记

#define blk_noprev(ptr)                                                        \
    ((bool)(blk_prevtail(ptr) & AREA_FLAG)) // 是否有上一个块
#define blk_nonext(ptr, size)                                                  \
    ((bool)(blk_nexthead(ptr, size) & AREA_FLAG)) // 是否有下一个块
