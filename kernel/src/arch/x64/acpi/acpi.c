...
void *find_table(const char *name) {
    ...
    uint64_t *t =
        (uint64_t *)((char *)xsdt + offsetof(XSDT, pointer_to_other_sdt));
    for (uint64_t i = 0; i < entry_count; i++) {
        uint64_t phys = (uint64_t)(*(t + i));
        uint64_t ptr = phys_to_virt(phys);
        map_page_range(get_current_page_dir(false), ptr, phys,
...
void acpi_init() {
    ...
    map_page_range(get_current_page_dir(false), (uint64_t)rsdp, rsdp_paddr,
                   DEFAULT_PAGE_SIZE, PT_FLAG_R | PT_FLAG_W);
    ...
    map_page_range(get_current_page_dir(false), (uint64_t)xsdt, xsdt_paddr,
                   DEFAULT_PAGE_SIZE, PT_FLAG_R | PT_FLAG_W);
