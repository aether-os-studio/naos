...
void uacpi_memcpy_zerout(void *dst, const void *src, uacpi_size dst_size,
                         uacpi_size src_size) {
    ...
    if (dst_size > bytes_to_copy)
        uacpi_memzero((uacpi_u8 *)dst + bytes_to_copy,
...
static void write_many(struct fmt_buf_state *fb_state, const uacpi_char *string,
                       uacpi_size count) {
    if (fb_state->bytes_written < fb_state->capacity) {
...
static void write_padding(struct fmt_buf_state *fb_state, struct fmt_spec *fm,
...
static void write_integer(struct fmt_buf_state *fb_state, struct fmt_spec *fm,
                          uacpi_u64 value) {
...
static uacpi_bool parse_number(const uacpi_char **fmt,
    ...
    ret =
        uacpi_string_to_integer(digits, num_digits, UACPI_BASE_DEC, out_value);
...
uacpi_i32 uacpi_vsnprintf(uacpi_char *buffer, uacpi_size capacity,
                          const uacpi_char *fmt, uacpi_va_list vlist) {
    ...
    while (*fmt) {
        ...
        } else if (!parse_number(&fmt, PARSE_NUMBER_MODE_MAYBE,
                                 &fm.min_width)) {
            return -1;
        }
        ...
        if (consume(&fmt, "s")) {
            ...
            for (i = 0; (!fm.has_precision || i < fm.precision) && string[i];
                 ++i)
...
uacpi_i32 uacpi_snprintf(uacpi_char *buffer, uacpi_size capacity,
