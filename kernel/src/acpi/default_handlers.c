...
static uacpi_namespace_node *find_pci_root(uacpi_namespace_node *node) {
    static const uacpi_char *pci_root_ids[] = {
        PCI_ROOT_PNP_ID, PCI_EXPRESS_ROOT_PNP_ID, UACPI_NULL};
    ...
    while (parent != uacpi_namespace_root()) {
        if (uacpi_device_matches_pnp_id(parent, pci_root_ids)) {
            uacpi_trace("found a PCI root node %.4s controlling region %.4s\n",
    ...
    uacpi_trace_region_error(node, "unable to find PCI root controlling",
...
static uacpi_status pci_region_attach(uacpi_region_attach_data *data) {
    ...
    if (uacpi_unlikely(device == UACPI_NULL)) {
        ...
        uacpi_trace_region_error(node, "unable to find device responsible for",
                                 ret);
        return ret;
    }
    ...
    uacpi_trace("detected PCI device %.4s@%04X:%02X:%02X:%01X\n",
                device->name.text, address.segment, address.bus, address.device,
                address.function);
...
static uacpi_status pci_region_do_rw(uacpi_region_op op,
...
static uacpi_status handle_pci_region(uacpi_region_op op,
...
static uacpi_status io_region_attach(uacpi_region_attach_data *data) {
    ...
    if (uacpi_unlikely_error(ret)) {
        uacpi_trace_region_error(data->region_node, "unable to map an IO", ret);
        uacpi_free(ctx, sizeof(*ctx));
        return ret;
    }
...
static uacpi_status memory_region_do_rw(uacpi_region_op op,
                                        uacpi_region_rw_data *data) {
    struct memory_region_ctx *ctx = data->region_context;
    uacpi_size offset;
    ...
    return op == UACPI_REGION_OP_READ
               ? uacpi_system_memory_read(ctx->virt, offset, data->byte_width,
                                          &data->value)
               : uacpi_system_memory_write(ctx->virt, offset, data->byte_width,
                                           data->value);
}

static uacpi_status handle_memory_region(uacpi_region_op op,
                                         uacpi_handle op_data) {
    switch (op) {
    case UACPI_REGION_OP_ATTACH:
...
static uacpi_status table_data_region_do_rw(uacpi_region_op op,
                                            uacpi_region_rw_data *data) {
    void *addr = UACPI_VIRT_ADDR_TO_PTR((uacpi_virt_addr)data->offset);

    return op == UACPI_REGION_OP_READ
               ? uacpi_system_memory_read(addr, 0, data->byte_width,
                                          &data->value)
               : uacpi_system_memory_write(addr, 0, data->byte_width,
                                           data->value);
}

static uacpi_status handle_table_data_region(uacpi_region_op op,
                                             uacpi_handle op_data) {
    switch (op) {
    case UACPI_REGION_OP_ATTACH:
    case UACPI_REGION_OP_DETACH:
...
static uacpi_status io_region_do_rw(uacpi_region_op op,
                                    uacpi_region_rw_data *data) {
    struct io_region_ctx *ctx = data->region_context;
    uacpi_u8 width;
    uacpi_size offset;
...
void uacpi_install_default_address_space_handlers(void) {
    ...
    uacpi_install_address_space_handler_with_flags(
        root, UACPI_ADDRESS_SPACE_SYSTEM_MEMORY, handle_memory_region,
        UACPI_NULL, UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);

    uacpi_install_address_space_handler_with_flags(
        root, UACPI_ADDRESS_SPACE_SYSTEM_IO, handle_io_region, UACPI_NULL,
        UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);

    uacpi_install_address_space_handler_with_flags(
        root, UACPI_ADDRESS_SPACE_PCI_CONFIG, handle_pci_region, UACPI_NULL,
        UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);

    uacpi_install_address_space_handler_with_flags(
        root, UACPI_ADDRESS_SPACE_TABLE_DATA, handle_table_data_region,
        UACPI_NULL, UACPI_ADDRESS_SPACE_HANDLER_DEFAULT);
}
