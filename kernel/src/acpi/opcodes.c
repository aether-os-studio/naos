...
#define PARSE_FIELD_ELEMENTS(parse_loop_pc)                                    \
    ...
    UACPI_PARSE_OP_IF_HAS_DATA, 44, /* Look at the first byte */               \
        UACPI_PARSE_OP_LOAD_IMM, 1,                                            \
                                                                               \
        /* ReservedField := 0x00 PkgLength */                                  \
        UACPI_PARSE_OP_IF_LAST_EQUALS, 0x00, 3, UACPI_PARSE_OP_PKGLEN,         \
        UACPI_PARSE_OP_JMP, parse_loop_pc,                                     \
                                                                               \
        /* AccessField := 0x01 AccessType AccessAttrib */                      \
        UACPI_PARSE_OP_IF_LAST_EQUALS, 0x01, 6, UACPI_PARSE_OP_LOAD_IMM, 1,    \
        UACPI_PARSE_OP_LOAD_IMM, 1, UACPI_PARSE_OP_JMP, parse_loop_pc,         \
                                                                               \
        /* ConnectField := <0x02 NameString> | <0x02 BufferData> */            \
        UACPI_PARSE_OP_IF_LAST_EQUALS, 0x02, 5,                                \
        UACPI_PARSE_OP_TERM_ARG_UNWRAP_INTERNAL, UACPI_PARSE_OP_TYPECHECK,     \
        UACPI_OBJECT_BUFFER, UACPI_PARSE_OP_JMP, parse_loop_pc,                \
                                                                               \
        /* ExtendedAccessField := 0x03 AccessType ExtendedAccessAttrib         \
         *                                        AccessLength */              \
        UACPI_PARSE_OP_IF_LAST_EQUALS, 0x03, 8, UACPI_PARSE_OP_LOAD_IMM, 1,    \
        UACPI_PARSE_OP_LOAD_IMM, 1, UACPI_PARSE_OP_LOAD_IMM, 1,                \
        UACPI_PARSE_OP_JMP, parse_loop_pc,                                     \
                                                                               \
         ...
         */                                                                    \
        UACPI_PARSE_OP_ITEM_POP, UACPI_PARSE_OP_AML_PC_DECREMENT,              \
        UACPI_PARSE_OP_CREATE_NAMESTRING, UACPI_PARSE_OP_PKGLEN,               \
        UACPI_PARSE_OP_OBJECT_ALLOC_TYPED, UACPI_OBJECT_FIELD_UNIT,            \
        UACPI_PARSE_OP_JMP, parse_loop_pc,                                     \
                                                                               \
        UACPI_PARSE_OP_INVOKE_HANDLER, UACPI_PARSE_OP_END

uacpi_u8 uacpi_field_op_decode_ops[] = {
    UACPI_PARSE_OP_TRACKED_PKGLEN, UACPI_PARSE_OP_EXISTING_NAMESTRING,
    UACPI_PARSE_OP_LOAD_IMM,       1,
...
static const uacpi_char *const pop_names[UACPI_PARSE_OP_MAX + 1] = {
    ...
    [POP(TERM_ARG_OR_NAMED_OBJECT_OR_UNRESOLVED)] =
        "TERM_ARG_OR_NAMED_OBJECT_OR_UNRESOLVED",
    [POP(OPERAND)] = "OPERAND",
    [POP(STRING)] = "STRING",
    ...
    [POP(CREATE_NAMESTRING_OR_NULL_IF_LOAD)] =
        "CREATE_NAMESTRING_OR_NULL_IF_LOAD",
    [POP(EXISTING_NAMESTRING)] = "EXISTING_NAMESTRING",
    [POP(EXISTING_NAMESTRING_OR_NULL)] = "EXISTING_NAMESTRING_OR_NULL",
    [POP(EXISTING_NAMESTRING_OR_NULL_IF_LOAD)] =
        "EXISTING_NAMESTRING_OR_NULL_IF_LOAD",
