...
static const uacpi_u8 aml_resource_to_type[256] = {
    ...
    [L(ACPI_RESOURCE_ADDRESS64_EXTENDED)] =
    ...
    [L(ACPI_RESOURCE_PIN_GROUP_FUNCTION)] =
        UACPI_AML_RESOURCE_PIN_GROUP_FUNCTION,
    [L(ACPI_RESOURCE_PIN_GROUP_CONFIGURATION)] =
...
static uacpi_size
extra_size_for_native_irq_or_dma(const struct uacpi_resource_spec *spec,
                                 void *data, uacpi_size size) {
    uacpi_u16 mask;
...
static uacpi_size
size_for_aml_start_dependent(const struct uacpi_resource_spec *spec,
                             uacpi_resource *resource) {
...
static uacpi_size
extra_size_for_native_vendor(const struct uacpi_resource_spec *spec, void *data,
                             uacpi_size size) {
    UACPI_UNUSED(spec);
    UACPI_UNUSED(data);
    return size;
...
static uacpi_size size_for_aml_vendor(const struct uacpi_resource_spec *spec,
    ...
    if (size > 7 || resource->type == UACPI_RESOURCE_TYPE_VENDOR_LARGE) {
        size += aml_resource_kind_to_header_size[UACPI_AML_RESOURCE_KIND_LARGE];
        ...
        size += aml_resource_kind_to_header_size[UACPI_AML_RESOURCE_KIND_SMALL];
    }
...
static uacpi_size
size_for_aml_address_or_clock_input(const struct uacpi_resource_spec *spec,
                                    uacpi_resource *resource) {
...
static uacpi_size
extra_size_for_extended_irq(const struct uacpi_resource_spec *spec, void *data,
                            uacpi_size size) {
    ...
    extra_size +=
        extra_size_for_resource_source(spec->aml_size, size - extra_size);

    return extra_size;
}

static uacpi_size
size_for_aml_extended_irq(const struct uacpi_resource_spec *spec,
                          uacpi_resource *resource) {
...
static uacpi_size
extra_size_for_native_gpio_or_pins(const struct uacpi_resource_spec *spec,
                                   void *data, uacpi_size size) {
...
static uacpi_size
size_for_aml_gpio_or_pins(const struct uacpi_resource_spec *spec,
                          uacpi_resource *resource) {
...
static uacpi_size
extra_size_for_native_pin_group(const struct uacpi_resource_spec *spec,
                                void *data, uacpi_size size) {
...
static const uacpi_u8
    aml_serial_resource_to_extra_aml_size[ACPI_SERIAL_TYPE_MAX + 1] = {
...
static const uacpi_u8
    aml_serial_resource_to_extra_native_size[ACPI_SERIAL_TYPE_MAX + 1] = {
...
static uacpi_size
extra_size_for_serial_connection(const struct uacpi_resource_spec *spec,
                                 void *data, uacpi_size size) {
...
static uacpi_size
aml_size_for_serial_connection(const struct uacpi_resource_spec *spec,
                               uacpi_resource *resource) {
    uacpi_size size;
...
#define OP(short_code, ...)                                                    \
    {.code = UACPI_RESOURCE_CONVERT_OPCODE_##short_code, __VA_ARGS__}
...
static const struct uacpi_resource_convert_instruction convert_irq_to_native[] =
    {
        ...
        OP(BIT_FIELD_1, AML_F(irq, flags), NATIVE_F(irq, wake_capability),
           IMM(5)),
        END(),
        OP(SET_TO_IMM, NATIVE_F(irq, length_kind),
...
static const struct uacpi_resource_convert_instruction convert_dma[] = {
    ...
    OP(BIT_FIELD_1, AML_F(dma, flags), NATIVE_F(dma, bus_master_status),
       IMM(2)),
...
static const struct uacpi_resource_convert_instruction convert_vendor_type0[] =
    {
        ...
        OP(FIELD_8, AML_F(vendor_defined_type0, byte_data),
           NATIVE_F(vendor, data)),
        END(),
};
...
static const struct uacpi_resource_convert_instruction convert_vendor_type1[] =
    {
        OP(LOAD_AML_SIZE_32, NATIVE_F(vendor, length)),
        OP(FIELD_8, AML_F(vendor_defined_type1, byte_data),
           NATIVE_F(vendor, data)),
        END(),
};
...
#define CONVERT_TYPE_SPECIFIC_FLAGS(addr_type)                                 \
    OP(LOAD_8_STORE, AML_F(addr_type, common.type),                            \
       NATIVE_F(addr_type, common.type)),                                      \
        ...
        OP(BIT_FIELD_1, AML_F(addr_type, common.type_flags),                   \
           NATIVE_F(addr_type, common.attribute.memory.write_status), IMM(0)), \
        OP(BIT_FIELD_2, AML_F(addr_type, common.type_flags),                   \
        ...
        END(), OP(SKIP_IF_NOT_EQUALS, ARG0(UACPI_RANGE_IO), IMM(4)),           \
        OP(BIT_FIELD_2, AML_F(addr_type, common.type_flags),                   \
        ...
        END(), /* Memory type that we don't know, just copy the byte */        \
...
#define CONVERT_GENERAL_ADDRESS_FLAGS(addr_type)                               \
    OP(BIT_FIELD_1, AML_F(addr_type, common.flags),                            \
...
static const struct uacpi_resource_convert_instruction convert_clock_input[] = {
    ...
    OP(FIELD_32, AML_F(clock_input, numerator),
       NATIVE_F(clock_input, numerator)),
    OP(FIELD_8, AML_F(clock_input, source_index),
       NATIVE_F(clock_input, source.index)),
...
#define DECODE_RES_PIN_TBL_AND_VENDOR_DATA(short_aml_name, res_opcode,         \
                                           offset_field, res_field)            \
...
static const struct uacpi_resource_convert_instruction
    convert_gpio_connection[] = {
        ...
        DECODE_RES_PIN_TBL_AND_VENDOR_DATA(gpio_connection, SOURCE_NO_INDEX,
                                           source_offset, source),
        OP(LOAD_8_STORE, AML_F(gpio_connection, type),
           NATIVE_F(gpio_connection, type)),
...
static const struct uacpi_resource_convert_instruction convert_pin_function[] =
    {
        ...
        DECODE_RES_PIN_TBL_AND_VENDOR_DATA(pin_function, SOURCE_NO_INDEX,
                                           source_offset, source),
        END(),
};
...
static const struct uacpi_resource_convert_instruction
    convert_pin_configuration[] = {
        ...
        DECODE_RES_PIN_TBL_AND_VENDOR_DATA(pin_configuration, SOURCE_NO_INDEX,
                                           source_offset, source),
        END(),
};
...
static const struct uacpi_resource_convert_instruction convert_pin_group[] = {
    ...
    OP(BIT_FIELD_1, AML_F(pin_group, flags), NATIVE_F(pin_group, direction),
       IMM(0)),
    DECODE_RES_PIN_TBL_AND_VENDOR_DATA(pin_group, LABEL, source_lable_offset,
                                       label),
    END(),
};
...
static const struct uacpi_resource_convert_instruction
    convert_generic_serial_bus[] = {
        ...
        OP(BIT_FIELD_1, AML_F(serial, flags), NATIVE_F(serial_bus_common, mode),
           IMM(0)),
        ...
        /*
         * Insert a trap to catch unimplemented types, this should be
         * unreachable because of validation earlier.
         */
        OP(UNREACHABLE),
};
...
#define DEFINE_SMALL_AML_RESOURCE(aml_type_enum, native_type_enum, aml_struct, \
                                  native_struct, ...)                          \
    [aml_type_enum] = {                                                        \
        .type = aml_type_enum,                                                 \
...
#define DEFINE_SMALL_AML_RESOURCE_NO_NATIVE_REPR(                              \
    ...
    [aml_type_enum] = {.type = aml_type_enum,                                  \
                       ...
                       .resource_kind = UACPI_AML_RESOURCE_KIND_SMALL,         \
                       .aml_size =                                             \
                           sizeof(aml_struct) - SMALL_ITEM_HEADER_SIZE,        \
                       .native_size = NATIVE_RESOURCE_HEADER_SIZE,             \
                       __VA_ARGS__}

#define DEFINE_LARGE_AML_RESOURCE(aml_type_enum, native_type_enum, aml_struct, \
                                  native_struct, ...)                          \
    [aml_type_enum] = {                                                        \
        .type = aml_type_enum,                                                 \
        .native_type = native_type_enum,                                       \
...
const struct uacpi_resource_spec aml_resources[UACPI_AML_RESOURCE_MAX + 1] = {
    DEFINE_SMALL_AML_RESOURCE(
        UACPI_AML_RESOURCE_IRQ, UACPI_RESOURCE_TYPE_IRQ,
        struct acpi_resource_irq, uacpi_resource_irq,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED_OR_ONE_LESS,
        .extra_size_for_native = extra_size_for_native_irq_or_dma,
        .size_for_aml = size_for_aml_irq, .to_native = convert_irq_to_native,
        .to_aml = convert_irq_to_aml, ),
    DEFINE_SMALL_AML_RESOURCE(
        UACPI_AML_RESOURCE_DMA, UACPI_RESOURCE_TYPE_DMA,
        struct acpi_resource_dma, uacpi_resource_dma,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED,
        .extra_size_for_native = extra_size_for_native_irq_or_dma,
        .to_native = convert_dma, .to_aml = convert_dma, ),
    DEFINE_SMALL_AML_RESOURCE(
        UACPI_AML_RESOURCE_START_DEPENDENT, UACPI_RESOURCE_TYPE_START_DEPENDENT,
        struct acpi_resource_start_dependent, uacpi_resource_start_dependent,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED_OR_ONE_LESS,
    ...
    DEFINE_SMALL_AML_RESOURCE_NO_NATIVE_REPR(
        ...
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED, ),
    DEFINE_SMALL_AML_RESOURCE(UACPI_AML_RESOURCE_IO, UACPI_RESOURCE_TYPE_IO,
                              struct acpi_resource_io, uacpi_resource_io,
                              .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED,
                              .to_native = convert_io, .to_aml = convert_io, ),
    DEFINE_SMALL_AML_RESOURCE(
        UACPI_AML_RESOURCE_FIXED_IO, UACPI_RESOURCE_TYPE_FIXED_IO,
        struct acpi_resource_fixed_io, uacpi_resource_fixed_io,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED,
        .to_native = convert_fixed_io, .to_aml = convert_fixed_io, ),
    DEFINE_SMALL_AML_RESOURCE(
        UACPI_AML_RESOURCE_FIXED_DMA, UACPI_RESOURCE_TYPE_FIXED_DMA,
        struct acpi_resource_fixed_dma, uacpi_resource_fixed_dma,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED,
        .to_native = convert_fixed_dma, .to_aml = convert_fixed_dma, ),
    DEFINE_SMALL_AML_RESOURCE(
        UACPI_AML_RESOURCE_VENDOR_TYPE0, UACPI_RESOURCE_TYPE_VENDOR_SMALL,
        struct acpi_resource_vendor_defined_type0, uacpi_resource_vendor,
    ...
    DEFINE_SMALL_AML_RESOURCE_NO_NATIVE_REPR(
        UACPI_AML_RESOURCE_END_TAG, UACPI_RESOURCE_TYPE_END_TAG,
        struct acpi_resource_end_tag,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED, ),
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_MEMORY24, UACPI_RESOURCE_TYPE_MEMORY24,
        struct acpi_resource_memory24, uacpi_resource_memory24,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED,
        .to_native = convert_memory24, .to_aml = convert_memory24, ),
    DEFINE_LARGE_AML_RESOURCE(UACPI_AML_RESOURCE_GENERIC_REGISTER,
                              UACPI_RESOURCE_TYPE_GENERIC_REGISTER,
    ...
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_VENDOR_TYPE1, UACPI_RESOURCE_TYPE_VENDOR_LARGE,
        struct acpi_resource_vendor_defined_type1, uacpi_resource_vendor,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_VARIABLE,
        .extra_size_for_native = extra_size_for_native_vendor,
        .size_for_aml = size_for_aml_vendor, .to_native = convert_vendor_type1,
        .to_aml = convert_vendor_type1, ),
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_MEMORY32, UACPI_RESOURCE_TYPE_MEMORY32,
        struct acpi_resource_memory32, uacpi_resource_memory32,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED,
        .to_native = convert_memory32, .to_aml = convert_memory32, ),
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_FIXED_MEMORY32, UACPI_RESOURCE_TYPE_FIXED_MEMORY32,
        struct acpi_resource_fixed_memory32, uacpi_resource_fixed_memory32,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_FIXED,
        .to_native = convert_fixed_memory32,
        .to_aml = convert_fixed_memory32, ),
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_ADDRESS32, UACPI_RESOURCE_TYPE_ADDRESS32,
        struct acpi_resource_address32, uacpi_resource_address32,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_VARIABLE,
        .extra_size_for_native = extra_size_for_native_address_or_clock_input,
        .size_for_aml = size_for_aml_address_or_clock_input,
        .to_native = convert_address32, .to_aml = convert_address32, ),
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_ADDRESS16, UACPI_RESOURCE_TYPE_ADDRESS16,
        struct acpi_resource_address16, uacpi_resource_address16,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_VARIABLE,
        .extra_size_for_native = extra_size_for_native_address_or_clock_input,
        .size_for_aml = size_for_aml_address_or_clock_input,
        .to_native = convert_address16, .to_aml = convert_address16, ),
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_EXTENDED_IRQ, UACPI_RESOURCE_TYPE_EXTENDED_IRQ,
        struct acpi_resource_extended_irq, uacpi_resource_extended_irq,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_VARIABLE,
    ...
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_ADDRESS64, UACPI_RESOURCE_TYPE_ADDRESS64,
        struct acpi_resource_address64, uacpi_resource_address64,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_VARIABLE,
        .extra_size_for_native = extra_size_for_native_address_or_clock_input,
        .size_for_aml = size_for_aml_address_or_clock_input,
        .to_native = convert_address64, .to_aml = convert_address64, ),
    DEFINE_LARGE_AML_RESOURCE(UACPI_AML_RESOURCE_ADDRESS64_EXTENDED,
                              UACPI_RESOURCE_TYPE_ADDRESS64_EXTENDED,
    ...
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_GPIO_CONNECTION, UACPI_RESOURCE_TYPE_GPIO_CONNECTION,
        struct acpi_resource_gpio_connection, uacpi_resource_gpio_connection,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_VARIABLE,
    ...
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_PIN_FUNCTION, UACPI_RESOURCE_TYPE_PIN_FUNCTION,
        struct acpi_resource_pin_function, uacpi_resource_pin_function,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_VARIABLE,
        .extra_size_for_native = extra_size_for_native_gpio_or_pins,
        .size_for_aml = size_for_aml_gpio_or_pins,
    ...
    DEFINE_LARGE_AML_RESOURCE(
        ...
        struct acpi_resource_serial, uacpi_resource_serial_bus_common,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_VARIABLE,
    ...
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_PIN_GROUP, UACPI_RESOURCE_TYPE_PIN_GROUP,
    ...
    DEFINE_LARGE_AML_RESOURCE(
        UACPI_AML_RESOURCE_CLOCK_INPUT, UACPI_RESOURCE_TYPE_CLOCK_INPUT,
        struct acpi_resource_clock_input, uacpi_resource_clock_input,
        .size_kind = UACPI_AML_RESOURCE_SIZE_KIND_VARIABLE,
        .extra_size_for_native = extra_size_for_native_address_or_clock_input,
        .size_for_aml = size_for_aml_address_or_clock_input,
        .to_native = convert_clock_input, .to_aml = convert_clock_input, ),
};

static enum uacpi_aml_resource get_aml_resource_type(uacpi_u8 raw_byte) {
    if (raw_byte & ACPI_LARGE_ITEM) {
        return aml_resource_to_type[LARGE_RESOURCE_BASE +
                                    (raw_byte & ACPI_LARGE_ITEM_NAME_MASK)];
    }

    return aml_resource_to_type[(raw_byte >> ACPI_SMALL_ITEM_NAME_IDX) &
                                ACPI_SMALL_ITEM_NAME_MASK];
}

static uacpi_status get_aml_resource_size(uacpi_u8 *data, uacpi_size bytes_left,
                                          uacpi_u16 *out_size) {
    uacpi_u16 size;
    ...
    if (*data & ACPI_LARGE_ITEM) {
        ...
        bytes_left -=
            aml_resource_kind_to_header_size[UACPI_AML_RESOURCE_KIND_LARGE];
    } else {
        size = *data & ACPI_SMALL_ITEM_LENGTH_MASK;
        bytes_left -=
            aml_resource_kind_to_header_size[UACPI_AML_RESOURCE_KIND_SMALL];
    }
...
uacpi_status
uacpi_for_each_aml_resource(uacpi_data_view buffer,
...
find_end(void *opaque, uacpi_u8 *data, uacpi_u16 resource_size,
         const struct uacpi_resource_spec *spec) {
...
static uacpi_size
native_size_for_aml_resource(uacpi_u8 *data, uacpi_u16 size,
                             const struct uacpi_resource_spec *spec) {
...
static uacpi_iteration_decision
conditional_continue(struct resource_conversion_ctx *ctx) {
...
#define CONVERSION_OPCODES_COMMON(native_buf)                                  \
    ...
    case UACPI_RESOURCE_CONVERT_OPCODE_LOAD_16_NATIVE: {                       \
        ...
        uacpi_memcpy_zerout(&accumulator, native_buf, sizeof(accumulator),     \
                            bytes);                                            \
        break;                                                                 \
    }                                                                          \
                                                                               \
...
#define CHECK_AML_OOB(offset, prefix, what)                                    \
    if (uacpi_unlikely(offset > ((uacpi_u32)aml_size + header_size))) {        \
        uacpi_error(prefix what " is OOB: %zu > %u\n", (uacpi_size)offset,     \
                    (uacpi_u32)aml_size + header_size);                        \
...
static uacpi_iteration_decision
do_aml_resource_to_native(void *opaque, uacpi_u8 *data, uacpi_u16 aml_size,
                          const struct uacpi_resource_spec *spec) {
    ...
    for (;;) {
        ...
        case UACPI_RESOURCE_CONVERT_OPCODE_PACKED_ARRAY_16: {
            ...
            if (insn->code == UACPI_RESOURCE_CONVERT_OPCODE_PACKED_ARRAY_16) {
                ...
                max_bit = 8;
                uacpi_memcpy_zerout(&value, src, sizeof(value),
                                    sizeof(uacpi_u8));
            }
        ...
        case UACPI_RESOURCE_CONVERT_OPCODE_LOAD_PIN_TABLE_LENGTH:
            uacpi_memcpy_zerout(&accumulator, src, sizeof(accumulator),
                                sizeof(uacpi_u16));
        ...
        case UACPI_RESOURCE_CONVERT_OPCODE_PIN_TABLE: {
             ...
             * copy the data there.
             */
            uacpi_memcpy(resource_end, data + base_aml_size_with_header,
                         accumulator);
            ...
            uacpi_memcpy(NATIVE_OFFSET(resource, insn->f3.arg2), &resource_end,
                         sizeof(void *));
            break;
        }
        ...
        case UACPI_RESOURCE_CONVERT_OPCODE_VENDOR_DATA: {
            ...
            uacpi_memcpy(NATIVE_OFFSET(resource, insn->f3.arg2), &native_dst,
                         sizeof(void *));
            break;
        }
        ...
        case UACPI_RESOURCE_CONVERT_OPCODE_SERIAL_TYPE_SPECIFIC: {
            ...
            if (uacpi_unlikely(type_length < extra_size)) {
                uacpi_error("invalid type-specific data length: %d, "
                            "expected at least %d\n",
...
static uacpi_iteration_decision
accumulate_native_buffer_size(void *opaque, uacpi_u8 *data,
                              uacpi_u16 resource_size,
                              const struct uacpi_resource_spec *spec) {
    struct resource_conversion_ctx *ctx = opaque;
...
static uacpi_status eval_resource_helper(uacpi_namespace_node *node,
    ...
    return uacpi_eval_simple_buffer(node, method, out_obj);
}
...
uacpi_status uacpi_native_resources_from_aml(uacpi_data_view aml_buffer,
    ...
    ret = uacpi_for_each_aml_resource(aml_buffer, accumulate_native_buffer_size,
                                      &ctx);
    if (uacpi_unlikely_error(ret))
        return ret;
    ...
    ret = uacpi_for_each_aml_resource(aml_buffer, do_aml_resource_to_native,
                                      &ctx);
    if (uacpi_unlikely_error(ret)) {
...
static uacpi_status
extract_native_resources_from_method(uacpi_namespace_node *device,
                                     const uacpi_char *method,
                                     uacpi_resources **out_resources) {
    uacpi_status ret;
    uacpi_object *obj;
...
static const struct uacpi_resource_spec *
resource_spec_from_native(uacpi_resource *resource) {
    return &aml_resources[native_resource_to_type[resource->type]];
}

static uacpi_size
aml_size_for_native_resource(uacpi_resource *resource,
                             const struct uacpi_resource_spec *spec) {
    return spec->size_for_aml ? spec->size_for_aml(spec, resource)
                              : aml_size_with_header(spec);
}

static uacpi_iteration_decision
do_native_resource_to_aml(void *opaque, uacpi_resource *resource) {
    struct resource_conversion_ctx *ctx = opaque;
    const struct uacpi_resource_spec *spec;
    const struct uacpi_resource_convert_instruction *insns, *insn;
    uacpi_u8 pc = 0;
    ...
    if (spec->resource_kind == UACPI_AML_RESOURCE_KIND_LARGE) {
        ...
        *dst_base = type_to_aml_resource[spec->type]
    ...
    for (;;) {
        ...
        case UACPI_RESOURCE_CONVERT_OPCODE_PIN_TABLE:
             ...
             */
            uacpi_memcpy(dst_base + base_aml_size_with_header, resource_end,
                         accumulator);
            break;
        ...
        case UACPI_RESOURCE_CONVERT_OPCODE_VENDOR_DATA: {
             ...
             * nonetheless.
             */
            uacpi_memcpy(dst + sizeof(uacpi_u16), &vendor_data_length,
                         sizeof(vendor_data_length));

            // Finally write the data itself
            uacpi_memcpy(dst_base + vendor_data_offset, vendor_data,
                         vendor_data_length);
            break;
        }
        ...
        case UACPI_RESOURCE_CONVERT_OPCODE_SERIAL_TYPE_SPECIFIC: {
            ...
            *dst = serial_type;
            ...
            if (uacpi_unlikely(resource->type !=
                ...
                uacpi_error("native serial resource type %d doesn't match "
                            "expected %d\n",
                            resource->type,
                            aml_serial_to_native_type(serial_type));
                ctx->st = UACPI_STATUS_INVALID_ARGUMENT;
            ...
            resource_end =
                PTR_AT(resource_end,
                       aml_serial_resource_to_extra_native_size[serial_type]);
            base_aml_size_with_header +=
                aml_serial_resource_to_extra_aml_size[serial_type];
...
static uacpi_status native_resources_to_aml(uacpi_resources *native_resources,
    ...
    ret = uacpi_for_each_resource(native_resources, do_native_resource_to_aml,
                                  &ctx);
...
static uacpi_iteration_decision
accumulate_aml_buffer_size(void *opaque, uacpi_resource *resource) {
    struct resource_conversion_ctx *ctx = opaque;
    const struct uacpi_resource_spec *spec;
    uacpi_size size_for_this;
...
uacpi_status uacpi_native_resources_to_aml(uacpi_resources *resources,
    ...
    ret = uacpi_for_each_resource(resources, accumulate_aml_buffer_size, &ctx);
    if (ret == UACPI_STATUS_NO_RESOURCE_END_TAG) {
        // An end tag is always included
        uacpi_resource end_tag = {.type = UACPI_RESOURCE_TYPE_END_TAG};
