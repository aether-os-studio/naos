...
void uacpi_logger_initialize(void) {
    ...
    if (!version_printed) {
        ...
        uacpi_info("starting uACPI, version %d.%d.%d\n", UACPI_MAJOR,
                   UACPI_MINOR, UACPI_PATCH);
    }
}
...
static void trace_table_load_failure(struct acpi_sdt_hdr *tbl,
                                     uacpi_log_level lvl, uacpi_status ret) {
...
uacpi_status uacpi_namespace_load(void) {
    ...
    for (cur_index = 0;; cur_index = tbl.index + 1) {
        ...
        ret =
            uacpi_table_load_with_cause(tbl.index, UACPI_TABLE_LOAD_CAUSE_INIT);
        if (uacpi_unlikely_error(ret)) {
    ...
    if (uacpi_unlikely(st.failure_counter != 0 ||
                       g_uacpi_rt_ctx.bad_timesource)) {
        uacpi_info("loaded %u AML blob%s (%u error%s)\n", st.load_counter,
        ...
        uacpi_info("successfully loaded %u AML blob%s, %" UACPI_PRIu64
                   " ops in "
...
static uacpi_status sta_eval(struct ns_init_context *ctx,
                             uacpi_namespace_node *node, uacpi_u32 *value) {
    uacpi_status ret;
...
do_sta_ini(void *opaque, uacpi_namespace_node *node, uacpi_u32 depth) {
...
uacpi_status uacpi_namespace_initialize(void) {
    ...
    uacpi_namespace_for_each_child(root, do_sta_ini, UACPI_NULL,
                                   UACPI_OBJECT_ANY_BIT, UACPI_MAX_DEPTH_ANY,
                                   &ctx);
    ...
    if (uacpi_likely(!g_uacpi_rt_ctx.bad_timesource)) {
        uacpi_info("namespace initialization done in %" UACPI_PRIu64 "ms: "
                   ...
                   UACPI_FMT64(elapsed_ms(begin_ts, end_ts)), ctx.devices,
                   ctx.thermal_zones);
    } else {
        uacpi_info(
...
uacpi_status uacpi_eval(uacpi_namespace_node *parent, const uacpi_char *path,
    ...
    if (path != UACPI_NULL) {
        ret = uacpi_namespace_node_resolve(parent, path, UACPI_SHOULD_LOCK_NO,
    ...
    if (obj->type != UACPI_OBJECT_METHOD) {
        ...
        ret =
            uacpi_object_assign(new_obj, obj, UACPI_ASSIGN_BEHAVIOR_DEEP_COPY);
        if (uacpi_unlikely_error(ret)) {
...
uacpi_status uacpi_eval_simple(uacpi_namespace_node *parent,
                               const uacpi_char *path, uacpi_object **ret) {
    return uacpi_eval(parent, path, UACPI_NULL, ret);
}

uacpi_status uacpi_execute(uacpi_namespace_node *parent, const uacpi_char *path,
                           const uacpi_object_array *args) {
...
#define TRACE_BAD_RET(path_fmt, type, ...)                                     \
    uacpi_warn("unexpected '%s' object returned by method " path_fmt           \
               ", expected type mask: %08X\n",                                 \
               uacpi_object_type_to_string(type), __VA_ARGS__)

#define TRACE_NO_RET(path_fmt, ...)                                            \
    uacpi_warn("no value returned from method " path_fmt                       \
               ", expected type mask: "                                        \
               "%08X\n",                                                       \
               __VA_ARGS__)
...
uacpi_status uacpi_eval_integer(uacpi_namespace_node *parent,
    ...
    ret = uacpi_eval_typed(parent, path, args, UACPI_OBJECT_INTEGER_BIT,
                           &int_obj);
    if (uacpi_unlikely_error(ret))
        return ret;
...
uacpi_status uacpi_eval_buffer_or_string(uacpi_namespace_node *parent,
                                         ...
                                         const uacpi_object_array *args,
                                         uacpi_object **ret) {
    return uacpi_eval_typed(parent, path, args,
...
uacpi_status uacpi_eval_simple_buffer_or_string(uacpi_namespace_node *parent,
                                                const uacpi_char *path,
                                                uacpi_object **ret) {
    return uacpi_eval_typed(parent, path, UACPI_NULL,
                            UACPI_OBJECT_BUFFER_BIT | UACPI_OBJECT_STRING_BIT,
                            ret);
}
...
uacpi_status uacpi_eval_simple_string(uacpi_namespace_node *parent,
                                      const uacpi_char *path,
                                      uacpi_object **ret) {
    return uacpi_eval_typed(parent, path, UACPI_NULL, UACPI_OBJECT_STRING_BIT,
                            ret);
}
...
uacpi_status uacpi_eval_simple_buffer(uacpi_namespace_node *parent,
                                      const uacpi_char *path,
                                      uacpi_object **ret) {
    return uacpi_eval_typed(parent, path, UACPI_NULL, UACPI_OBJECT_BUFFER_BIT,
                            ret);
}
...
uacpi_status uacpi_eval_simple_package(uacpi_namespace_node *parent,
                                       const uacpi_char *path,
                                       uacpi_object **ret) {
    return uacpi_eval_typed(parent, path, UACPI_NULL, UACPI_OBJECT_PACKAGE_BIT,
                            ret);
}
