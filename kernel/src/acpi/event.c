...
static uacpi_status initialize_fixed_events(void) {
    ...
    for (i = 0; i < UACPI_FIXED_EVENT_MAX; ++i) {
        uacpi_write_register_field(fixed_events[i].enable_field,
...
uacpi_status uacpi_clear_fixed_event(uacpi_fixed_event event) {
    ...
    return uacpi_write_register_field(fixed_events[event].status_field,
                                      ACPI_PM1_STS_CLEAR);
}

static uacpi_interrupt_ret dispatch_fixed_event(const struct fixed_event *ev,
...
static uacpi_status set_gpe_state(struct gp_event *event,
                                  enum gpe_state state) {
    uacpi_status ret;
...
out_no_unlock:
     ...
     */
    ret = uacpi_kernel_schedule_work(UACPI_WORK_NOTIFICATION, async_restore_gpe,
                                     event);
    if (uacpi_unlikely_error(ret)) {
...
static uacpi_interrupt_ret dispatch_gpe(uacpi_namespace_node *device_node,
    ...
    if (event->handler_type == GPE_HANDLER_TYPE_NATIVE_HANDLER_RAW) {
        return event->native_handler->cb(event->native_handler->ctx,
                                         device_node, event->idx);
    }
    ...
    case GPE_HANDLER_TYPE_NATIVE_HANDLER:
        int_ret = event->native_handler->cb(event->native_handler->ctx,
                                            device_node, event->idx);
    ...
    case GPE_HANDLER_TYPE_AML_HANDLER:
    case GPE_HANDLER_TYPE_IMPLICIT_NOTIFY:
        ret = uacpi_kernel_schedule_work(UACPI_WORK_GPE_EXECUTION,
                                         async_run_gpe_handler, event);
        if (uacpi_unlikely_error(ret)) {
...
static uacpi_status maybe_dispatch_gpe(uacpi_namespace_node *gpe_device,
                                       struct gp_event *event) {
    uacpi_status ret;
    struct gpe_register *reg = event->reg;
    uacpi_u64 status;
...
static uacpi_status
find_or_create_gpe_interrupt_ctx(uacpi_u32 irq,
                                 struct gpe_interrupt_ctx **out_ctx) {
    uacpi_status ret;
    ...
    if (irq != g_uacpi_rt_ctx.fadt.sci_int) {
        ret = uacpi_kernel_install_interrupt_handler(irq, handle_gpes, entry,
                                                     &entry->irq_handle);
        if (uacpi_unlikely_error(ret)) {
...
static uacpi_status gpe_block_apply_action(struct gpe_block *block,
    ...
    if (block->irq_ctx) {
        ...
        if (ctx->gpe_head == UACPI_NULL) {
            ...
            if (ctx->irq != g_uacpi_rt_ctx.fadt.sci_int) {
                uacpi_kernel_uninstall_interrupt_handler(handle_gpes,
                                                         ctx->irq_handle);
            }
...
static uacpi_iteration_decision do_match_gpe_methods(uacpi_handle opaque,
...
static uacpi_status create_gpe_block(uacpi_namespace_node *device_node,
                                     uacpi_u32 irq, uacpi_u16 base_idx,
                                     uacpi_u64 address,
    ...
    block->registers =
        uacpi_kernel_alloc_zeroed(num_registers * sizeof(*block->registers));
    if (uacpi_unlikely(block->registers == UACPI_NULL))
        goto error_out;

    block->num_events = num_registers * EVENTS_PER_GPE_REGISTER;
    block->events =
        uacpi_kernel_alloc_zeroed(block->num_events * sizeof(*block->events));
    ...
    uacpi_namespace_do_for_each_child(
        device_node, do_match_gpe_methods, UACPI_NULL, UACPI_OBJECT_METHOD_BIT,
        UACPI_MAX_DEPTH_ANY, UACPI_SHOULD_LOCK_YES, UACPI_PERMANENT_ONLY_YES,
        &match_ctx);
...
static void for_each_gpe_block(gpe_block_iteration_callback cb,
                               uacpi_handle handle) {
...
static uacpi_iteration_decision do_find_gpe(struct gpe_block *block,
                                            uacpi_handle opaque) {
...
static struct gp_event *get_gpe(uacpi_namespace_node *gpe_device,
                                uacpi_u16 idx) {
...
static uacpi_status gpe_add_user(struct gp_event *event,
                                 enum event_clear_if_first clear_if_first) {
    uacpi_status ret = UACPI_STATUS_OK;
...
const uacpi_char *
uacpi_gpe_triggering_to_string(uacpi_gpe_triggering triggering) {
    switch (triggering) {
...
static uacpi_status gpe_mask_unmask(struct gp_event *event,
                                    uacpi_bool should_mask) {
    struct gpe_register *reg;
    uacpi_u8 mask;
    ...
    if (should_mask) {
         ...
         * disable this event knowing it won't be re-enabled by a racing IRQ.
         */
...
static uacpi_iteration_decision do_initialize_gpe_block(struct gpe_block *block,
                                                        uacpi_handle opaque) {
    uacpi_status ret;
    ...
    if (count_enabled) {
        uacpi_info("enabled %zu GPEs in block %.4s@[%d->%d]\n", count_enabled,
                   block->device_node->name.text, block->base_idx,
                   block->base_idx + block->num_events);
    }
    return UACPI_ITERATION_DECISION_CONTINUE;
}
...
static uacpi_status
sanitize_device_and_find_gpe(uacpi_namespace_node **gpe_device, uacpi_u16 idx,
                             struct gp_event **out_event) {
    if (*gpe_device == UACPI_NULL) {
        *gpe_device =
            uacpi_namespace_get_predefined(UACPI_PREDEFINED_NAMESPACE_GPE);
    }
...
static uacpi_status do_install_gpe_handler(uacpi_namespace_node *gpe_device,
                                           uacpi_u16 idx,
                                           uacpi_gpe_triggering triggering,
                                           enum gpe_handler_type type,
...
uacpi_status uacpi_install_gpe_handler(uacpi_namespace_node *gpe_device,
                                       ...
                                       uacpi_gpe_handler handler,
                                       uacpi_handle ctx) {
    return do_install_gpe_handler(gpe_device, idx, triggering,
                                  GPE_HANDLER_TYPE_NATIVE_HANDLER, handler,
                                  ctx);
}
...
uacpi_status uacpi_install_gpe_handler_raw(uacpi_namespace_node *gpe_device,
                                           ...
                                           uacpi_handle ctx) {
    return do_install_gpe_handler(gpe_device, idx, triggering,
                                  GPE_HANDLER_TYPE_NATIVE_HANDLER_RAW, handler,
                                  ctx);
}
...
static uacpi_status gpe_suspend_resume(uacpi_namespace_node *gpe_device,
                                       uacpi_u16 idx, enum gpe_state state) {
    uacpi_status ret;
    struct gp_event *event;
...
static uacpi_status gpe_get_mask_unmask(uacpi_namespace_node *gpe_device,
                                        uacpi_u16 idx, uacpi_bool should_mask) {
    uacpi_status ret;
    struct gp_event *event;
...
uacpi_status uacpi_setup_gpe_for_wake(uacpi_namespace_node *gpe_device,
    ...
    if (wake_device != UACPI_NULL) {
        ...
        if (!is_dev) {
            ret = uacpi_namespace_node_is(wake_device, UACPI_OBJECT_DEVICE,
                                          &is_dev);
            if (uacpi_unlikely_error(ret))
                return ret;
        }
...
static uacpi_status
gpe_enable_disable_for_wake(uacpi_namespace_node *gpe_device, uacpi_u16 idx,
                            uacpi_bool enabled) {
    uacpi_status ret;
    struct gp_event *event;
    struct gpe_register *reg;
...
static uacpi_iteration_decision do_for_all_gpes(struct gpe_block *block,
                                                uacpi_handle opaque) {
...
static uacpi_status initialize_gpes(void) {
    ...
    if (fadt->x_gpe0_blk.address && fadt->gpe0_blk_len) {
        ...
        ret = create_gpe_block(gpe_node, fadt->sci_int, 0,
                               fadt->x_gpe0_blk.address,
    ...
    if (fadt->x_gpe1_blk.address && fadt->gpe1_blk_len) {
        ...
        ret = create_gpe_block(gpe_node, fadt->sci_int, fadt->gpe1_base,
                               fadt->x_gpe1_blk.address,
...
uacpi_status uacpi_install_gpe_block(uacpi_namespace_node *gpe_device,
                                     ...
                                     uacpi_u16 num_registers, uacpi_u32 irq) {
    uacpi_status ret;
    uacpi_bool is_dev;
    ...
    ret = create_gpe_block(gpe_device, irq, 0, address, address_space,
                           num_registers);
...
uacpi_status uacpi_initialize_events(void) {
    ...
    ret = uacpi_install_fixed_event_handler(UACPI_FIXED_EVENT_GLOBAL_LOCK,
                                            handle_global_lock, UACPI_NULL);
...
void uacpi_deinitialize_events(void) {
    ...
    if (g_uacpi_rt_ctx.sci_handle_valid) {
        uacpi_kernel_uninstall_interrupt_handler(handle_sci,
                                                 g_uacpi_rt_ctx.sci_handle);
...
uacpi_status uacpi_gpe_info(uacpi_namespace_node *gpe_device, uacpi_u16 idx,
                            uacpi_event_info *out_info) {
    uacpi_status ret;
    struct gp_event *event;
    struct gpe_register *reg;
...
#define PM1_STATUS_BITS                                                        \
    (ACPI_PM1_STS_TMR_STS_MASK | ACPI_PM1_STS_BM_STS_MASK |                    \
     ACPI_PM1_STS_GBL_STS_MASK | ACPI_PM1_STS_PWRBTN_STS_MASK |                \
     ACPI_PM1_STS_SLPBTN_STS_MASK | ACPI_PM1_STS_RTC_STS_MASK |                \
