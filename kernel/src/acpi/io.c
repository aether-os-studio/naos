...
static void cut_misaligned_tail(uacpi_u8 *data, uacpi_size offset,
...
static void do_misaligned_buffer_read(const uacpi_buffer_field *field,
                                      uacpi_u8 *dst) {
...
static void do_write_misaligned_buffer_field(uacpi_buffer_field *field,
...
void uacpi_write_buffer_field(uacpi_buffer_field *field, const void *src,
                              uacpi_size size) {
    if (!(field->bit_index & 7)) {
...
static uacpi_status access_field_unit(uacpi_field_unit *field, uacpi_u32 offset,
                                      uacpi_region_op op,
                                      union uacpi_opregion_io_data data) {
    uacpi_status ret = UACPI_STATUS_OK;

    if (field->lock_rule) {
        ret = uacpi_acquire_aml_mutex(g_uacpi_rt_ctx.global_lock_mutex, 0xFFFF);
    ...
    case UACPI_FIELD_UNIT_KIND_BANK:
        ret = uacpi_write_field_unit(field->bank_selection, &field->bank_value,
                                     sizeof(field->bank_value), UACPI_NULL);
        break;
    case UACPI_FIELD_UNIT_KIND_NORMAL:
        break;
    case UACPI_FIELD_UNIT_KIND_INDEX:
        ret = uacpi_write_field_unit(field->index, &offset, sizeof(offset),
                                     UACPI_NULL);
        if (uacpi_unlikely_error(ret))
            goto out;
        ...
        case UACPI_REGION_OP_READ:
            ret = uacpi_read_field_unit(field->data, data.integer,
                                        field->access_width_bytes, UACPI_NULL);
            break;
        case UACPI_REGION_OP_WRITE:
            ret = uacpi_write_field_unit(field->data, data.integer,
                                         field->access_width_bytes, UACPI_NULL);
            break;
        default:
...
static uacpi_status wtr_buffer_size(uacpi_field_unit *field,
    ...
    case UACPI_ADDRESS_SPACE_SMBUS: {
        ...
        default:
            uacpi_error("unsupported field@%p access attribute %d\n", field,
                        field->attributes);
...
static uacpi_status handle_special_field(uacpi_field_unit *field,
                                         uacpi_data_view buf,
                                         uacpi_region_op op,
    ...
    *did_handle = UACPI_FALSE;
    ...
    if (uacpi_unlikely(obj == UACPI_NULL)) {
        ret = UACPI_STATUS_INVALID_ARGUMENT;
        uacpi_trace_region_error(field->region, "attempted access to deleted",
                                 ret);
        goto out_handled;
    }
    ...
    case UACPI_ADDRESS_SPACE_GENERAL_PURPOSE_IO:
        if (op == UACPI_REGION_OP_WRITE) {
            uacpi_memcpy_zerout(&in_out, buf.const_data, sizeof(in_out),
                                buf.length);
        }
...
do_wtr:
    ...
    uacpi_memcpy_zerout(wtr_buffer.data, buf.const_data, wtr_buffer.length,
                        buf.length);
    data.buffer = wtr_buffer;
    ret = access_field_unit(field, field->byte_offset, op, data);
    if (uacpi_unlikely_error(ret)) {
...
static uacpi_status do_read_misaligned_field_unit(uacpi_field_unit *field,
                                                  uacpi_u8 *dst,
                                                  uacpi_size size) {
    uacpi_status ret;
    ...
    reads_to_do =
        UACPI_ALIGN_UP(field->bit_offset_within_first_byte + field->bit_length,
                       width_access_bits, uacpi_u32);
    ...
    while (reads_to_do-- > 0) {
        ...
        src_span.length =
            UACPI_MIN(bits_left, width_access_bits - src_span.index);

        data.integer = &out;
        ret = access_field_unit(field, byte_offset, UACPI_REGION_OP_READ, data);
        if (uacpi_unlikely_error(ret))
            return ret;
...
uacpi_status uacpi_read_field_unit(uacpi_field_unit *field, void *dst,
                                   uacpi_size size,
                                   uacpi_data_view *wtr_response) {
    uacpi_status ret;
    ...
    ret = handle_special_field(field, data_view, UACPI_REGION_OP_READ,
    ...
    if (field->bit_offset_within_first_byte == 0 &&
        ...
        data.integer = &out;
        ret = access_field_unit(field, field->byte_offset, UACPI_REGION_OP_READ,
                                data);
        if (uacpi_unlikely_error(ret))
            return ret;
...
static uacpi_status write_generic_field_unit(uacpi_field_unit *field,
                                             const void *src, uacpi_size size) {
    uacpi_status ret;
    ...
    while (bits_left) {
        ...
        in = 0;
        dst_span.length =
            UACPI_MIN(width_access_bits - dst_span.index, bits_left);

        if (dst_span.index != 0 || dst_span.length < width_access_bits) {
            switch (field->update_rule) {
            case UACPI_UPDATE_RULE_PRESERVE:
                data.integer = &in;
                ret = access_field_unit(field, byte_offset,
                                        UACPI_REGION_OP_READ, data);
                if (uacpi_unlikely_error(ret))
                    return ret;
                break;
        ...
        ret =
            access_field_unit(field, byte_offset, UACPI_REGION_OP_WRITE, data);
        if (uacpi_unlikely_error(ret))
            return ret;
...
uacpi_status uacpi_write_field_unit(uacpi_field_unit *field, const void *src,
                                    uacpi_size size,
                                    uacpi_data_view *wtr_response) {
    uacpi_status ret;
    uacpi_bool did_handle;
    ...
    ret = handle_special_field(field, data_view, UACPI_REGION_OP_WRITE,
                               wtr_response, &did_handle);
    if (did_handle)
        return ret;
...
static uacpi_u8 gas_get_access_bit_width(const struct acpi_gas *gas) {
    ...
    if (gas->register_bit_offset == 0 &&
        ...
        if (access_bit_width <= 8) {
             ...
             */
            while (!UACPI_IS_ALIGNED(gas->address, access_bit_width / 8,
                                     uacpi_u64))
                access_bit_width /= 2;
        }
    }
...
static uacpi_status gas_validate(const struct acpi_gas *gas,
                                 uacpi_u8 *access_bit_width,
                                 uacpi_u8 *bit_width) {
    ...
    *access_bit_width = gas_get_access_bit_width(gas);
    ...
    if (uacpi_unlikely(aligned_width > 64)) {
        uacpi_warn("GAS register total width is too large: %zu\n", total_width);
        return UACPI_STATUS_UNIMPLEMENTED;
    }
...
uacpi_status uacpi_map_gas_noalloc(const struct acpi_gas *gas,
    ...
    if (gas->address_space_id == UACPI_ADDRESS_SPACE_SYSTEM_MEMORY) {
        ...
        ret = uacpi_kernel_io_map(gas->address, total_width / 8,
                                  &out_mapped->mapping);
        if (uacpi_unlikely_error(ret))
            return ret;
...
uacpi_status uacpi_system_memory_read(void *ptr, uacpi_size offset,
...
uacpi_status uacpi_system_memory_write(void *ptr, uacpi_size offset,
...
uacpi_status uacpi_system_io_read(uacpi_handle handle, uacpi_size offset,
                                  uacpi_u8 width, uacpi_u64 *out) {
    uacpi_status ret;
    ...
    case 4:
        ...
        break;
    default:
        uacpi_error("invalid SystemIO read %p@%zu width=%d\n", handle, offset,
                    width);
        return UACPI_STATUS_INVALID_ARGUMENT;
    }
...
uacpi_status uacpi_system_io_write(uacpi_handle handle, uacpi_size offset,
                                   uacpi_u8 width, uacpi_u64 in) {
    uacpi_status ret;
    ...
    case 4:
        ...
        break;
    default:
        uacpi_error("invalid SystemIO write %p@%zu width=%d\n", handle, offset,
                    width);
        return UACPI_STATUS_INVALID_ARGUMENT;
    }
...
uacpi_status uacpi_pci_read(uacpi_handle handle, uacpi_size offset,
                            uacpi_u8 width, uacpi_u64 *out) {
    uacpi_status ret;
    union integer_data data = {
        .qword = 0,
    ...
    case 4:
        ...
        break;
    default:
        uacpi_error("invalid PCI_Config read %p@%zu width=%d\n", handle, offset,
                    width);
        return UACPI_STATUS_INVALID_ARGUMENT;
    }
...
uacpi_status uacpi_pci_write(uacpi_handle handle, uacpi_size offset,
                             uacpi_u8 width, uacpi_u64 in) {
    uacpi_status ret;
    ...
    case 4:
        ...
        break;
    default:
        uacpi_error("invalid PCI_Config write %p@%zu width=%d\n", handle,
                    offset, width);
        return UACPI_STATUS_INVALID_ARGUMENT;
    }
