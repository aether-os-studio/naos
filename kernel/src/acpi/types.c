...
const uacpi_char *
uacpi_address_space_to_string(enum uacpi_address_space space) {
    switch (space) {
...
uacpi_bool uacpi_package_fill(uacpi_package *pkg, uacpi_size num_elements,
    ...
    pkg->objects =
        uacpi_kernel_alloc_zeroed(num_elements * sizeof(uacpi_handle));
...
static void free_address_space_handlers(uacpi_address_space_handler *handler) {
    ...
    while (handler) {
        ...
        uacpi_shareable_unref_and_delete_if_last(handler,
...
static void free_field_unit(uacpi_handle handle) {
    ...
    case UACPI_FIELD_UNIT_KIND_BANK:
        ...
        uacpi_shareable_unref_and_delete_if_last(field_unit->bank_selection,
                                                 free_field_unit);
        break;
    case UACPI_FIELD_UNIT_KIND_INDEX:
        uacpi_shareable_unref_and_delete_if_last(field_unit->index,
                                                 free_field_unit);
        uacpi_shareable_unref_and_delete_if_last(field_unit->data,
                                                 free_field_unit);
        break;
    default:
        break;
...
static uacpi_status buffer_alloc_and_store(uacpi_object *obj,
                                           uacpi_size buf_size, const void *src,
                                           uacpi_size src_size) {
...
DYNAMIC_ARRAY_WITH_INLINE_STORAGE_IMPL(pkg_copy_reqs, struct pkg_copy_req,
                                       static)

static uacpi_bool pkg_copy_reqs_push(struct pkg_copy_reqs *reqs,
...
static uacpi_status deep_copy_package_no_recurse(uacpi_object *dst,
    ...
    if (uacpi_unlikely(
            !package_alloc(dst, src->count, UACPI_PREALLOC_OBJECTS_YES)))
        return UACPI_STATUS_OUT_OF_MEMORY;
...
uacpi_object_type uacpi_object_get_type(uacpi_object *obj) { return obj->type; }
...
uacpi_status uacpi_object_get_string(uacpi_object *obj, uacpi_data_view *out) {
    return uacpi_object_do_get_string_or_buffer(obj, out,
...
uacpi_status
uacpi_object_resolve_as_aml_namepath(uacpi_object *obj,
...
static uacpi_status uacpi_object_do_assign_buffer(uacpi_object *obj,
    ...
    if (type == UACPI_OBJECT_STRING &&
        (in.length == 0 || in.const_bytes[in.length - 1] != 0x00))
        dst_buf_size++;

    ret = buffer_alloc_and_store(&tmp_obj, dst_buf_size, in.const_bytes,
                                 in.length);
    if (uacpi_unlikely_error(ret))
        return ret;

    ret =
        uacpi_object_assign(obj, &tmp_obj, UACPI_ASSIGN_BEHAVIOR_SHALLOW_COPY);
...
static uacpi_object *
uacpi_object_do_create_string_or_buffer(uacpi_data_view view,
                                        uacpi_object_type type) {
    uacpi_status ret;
    uacpi_object *obj;
...
uacpi_status uacpi_object_assign_reference(uacpi_object *obj,
    ...
    ret = uacpi_object_assign(obj, &object, UACPI_ASSIGN_BEHAVIOR_DEEP_COPY);
    if (uacpi_unlikely_error(ret))
        return ret;
...
uacpi_status
uacpi_object_get_power_resource_info(uacpi_object *obj,
...
uacpi_status uacpi_object_assign_package(uacpi_object *obj,
    ...
    if (uacpi_unlikely(
            !package_alloc(&tmp_obj, in.count, UACPI_PREALLOC_OBJECTS_NO)))
        return UACPI_STATUS_OUT_OF_MEMORY;
...
struct uacpi_object *
uacpi_create_internal_reference(enum uacpi_reference_kind kind,
                                uacpi_object *child) {
    uacpi_object *ret;
