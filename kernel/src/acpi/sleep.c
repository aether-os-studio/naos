...
#define CALL_SLEEP_FN(name, state)                                             \
    (uacpi_is_hardware_reduced() ? name##_hw_reduced(state)                    \
                                 : name##_hw_full(state))
#else
...
static uacpi_status enter_sleep_state_hw_full(uacpi_u8 state) {
    ...
    ret = uacpi_write_register_field(UACPI_REGISTER_FIELD_WAK_STS,
                                     ACPI_PM1_STS_CLEAR);
    ...
    pm1a &=
        ~((uacpi_u64)(ACPI_PM1_CNT_SLP_TYP_MASK | ACPI_PM1_CNT_SLP_EN_MASK));
    pm1b = pm1a;
    ...
    do {
        ret = uacpi_read_register_field(UACPI_REGISTER_FIELD_WAK_STS,
                                        &wake_status);
        if (uacpi_unlikely_error(ret))
            return ret;
    } while (wake_status != 1);
...
static uacpi_status wake_from_sleep_state_hw_full(uacpi_u8 state) {
    ...
    uacpi_write_register_field(UACPI_REGISTER_FIELD_WAK_STS,
                               ACPI_PM1_STS_CLEAR);
...
static uacpi_status get_slp_type_for_state(uacpi_u8 state, uacpi_u8 *a,
                                           uacpi_u8 *b) {
    uacpi_char path[] = "_S0";
    uacpi_status ret;
    ...
    ret = uacpi_eval_typed(uacpi_namespace_root(), path, UACPI_NULL,
    ...
    default:
        ...
        if (uacpi_unlikely(obj0->type != UACPI_OBJECT_INTEGER ||
            ...
            uacpi_error("invalid object type when evaluating %s: "
                        "pkg[0] => %s, pkg[1] => %s\n",
...
static uacpi_status eval_sleep_helper(uacpi_namespace_node *parent,
                                      const uacpi_char *path, uacpi_u8 value) {
    uacpi_object *arg;
...
static uacpi_status eval_sst(uacpi_u8 value) {
    return eval_sleep_helper(
        uacpi_namespace_get_predefined(UACPI_PREDEFINED_NAMESPACE_SI), "_SST",
        value);
}
...
uacpi_status uacpi_prepare_for_sleep_state(enum uacpi_sleep_state state_enum) {
    ...
    ret = get_slp_type_for_state(state, &g_uacpi_rt_ctx.last_sleep_typ_a,
                                 &g_uacpi_rt_ctx.last_sleep_typ_b);
    if (ret != UACPI_STATUS_OK)
        return ret;

    ret = get_slp_type_for_state(0, &g_uacpi_rt_ctx.s0_sleep_typ_a,
...
static uacpi_status enter_sleep_state_hw_reduced(uacpi_u8 state) {
    ...
    ret = uacpi_write_register_field(UACPI_REGISTER_FIELD_HWR_WAK_STS,
                                     ACPI_SLP_STS_CLEAR);
    if (uacpi_unlikely_error(ret))
        return ret;

    sleep_control =
        make_hw_reduced_sleep_control(g_uacpi_rt_ctx.last_sleep_typ_a);
     ...
     */
    do {
        ret = uacpi_read_register_field(UACPI_REGISTER_FIELD_HWR_WAK_STS,
                                        &wake_status);
        if (uacpi_unlikely_error(ret))
            return ret;
    } while (wake_status != 1);
...
static uacpi_status
prepare_for_wake_from_sleep_state_hw_reduced(uacpi_u8 state) {
    uacpi_u8 sleep_control;
    UACPI_UNUSED(state);
    ...
    sleep_control =
        make_hw_reduced_sleep_control(g_uacpi_rt_ctx.s0_sleep_typ_a);
...
static uacpi_status wake_from_sleep_state_hw_reduced(uacpi_u8 state) {
    ...
    // Apparently some BIOSes expect us to clear this, so do it
    uacpi_write_register_field(UACPI_REGISTER_FIELD_HWR_WAK_STS,
                               ACPI_SLP_STS_CLEAR);
...
uacpi_status
uacpi_prepare_for_wake_from_sleep_state(uacpi_sleep_state state_enum) {
    uacpi_u8 state = state_enum;
...
uacpi_status uacpi_reboot(void) {
    ...
    case UACPI_ADDRESS_SPACE_PCI_CONFIG: {
        ...
        ret = uacpi_kernel_pci_write8(pci_dev, reset_reg->address & 0xFFFF,
                                      fadt->reset_value);
        break;
    }
    default:
