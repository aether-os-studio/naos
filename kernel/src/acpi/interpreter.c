...
DYNAMIC_ARRAY_WITH_INLINE_STORAGE_IMPL(op_context_array, struct op_context,
                                       static)

static struct op_context *
op_context_array_one_before_last(struct op_context_array *arr) {
    uacpi_size size;
...
DYNAMIC_ARRAY_WITH_INLINE_STORAGE_IMPL(code_block_array, struct code_block,
                                       static)

DYNAMIC_ARRAY_WITH_INLINE_STORAGE(held_mutexes_array, uacpi_mutex *, 8)
DYNAMIC_ARRAY_WITH_INLINE_STORAGE_IMPL(held_mutexes_array, uacpi_mutex *,
                                       static)

static uacpi_status held_mutexes_array_push(struct held_mutexes_array *arr,
                                            uacpi_mutex *mutex) {
    uacpi_mutex **slot;
...
static void held_mutexes_array_remove_idx(struct held_mutexes_array *arr,
    ...
    if (i >= size) {
        ...
        uacpi_memmove(&arr->dynamic_storage[i], &arr->dynamic_storage[i + 1],
    ...
    size -= i + 1;
    uacpi_memmove(&arr->inline_storage[i], &arr->inline_storage[i + 1],
                  size * sizeof(arr->inline_storage[0]));
    ...
    if (size > i) {
        ...
        uacpi_memmove(&arr->dynamic_storage[0], &arr->dynamic_storage[1],
                      size * sizeof(arr->inline_storage[0]));
    }
...
static uacpi_status
held_mutexes_array_remove_and_release(struct held_mutexes_array *arr,
                                      uacpi_mutex *mutex,
...
DYNAMIC_ARRAY_WITH_INLINE_STORAGE(temp_namespace_node_array,
                                  uacpi_namespace_node *, 8)
DYNAMIC_ARRAY_WITH_INLINE_STORAGE_IMPL(temp_namespace_node_array,
                                       uacpi_namespace_node *, static)

static uacpi_status
temp_namespace_node_array_push(struct temp_namespace_node_array *arr,
...
DYNAMIC_ARRAY_WITH_INLINE_STORAGE_IMPL(call_frame_array, struct call_frame,
                                       static)

static struct call_frame *
call_frame_array_one_before_last(struct call_frame_array *arr) {
    uacpi_size size;
...
static uacpi_status name_string_to_path(struct call_frame *frame,
                                        uacpi_size offset,
...
static uacpi_status
resolve_name_string(struct call_frame *frame, enum resolve_behavior behavior,
...
static uacpi_status handle_buffer(struct execution_context *ctx) {
    ...
    if (uacpi_unlikely(declared_size->integer > 0xE0000000)) {
        uacpi_error("buffer is too large (%" UACPI_PRIu64
                    "), assuming corrupted "
                    "bytestream\n",
    ...
    if (uacpi_unlikely(init_size > buffer_size)) {
        uacpi_error("too many buffer initializers: %u (size is %u)\n",
...
static uacpi_status handle_package(struct execution_context *ctx) {
    ...
    if (op_ctx->op->code == UACPI_AML_OP_VarPackageOp) {
        ...
        if (uacpi_unlikely(var_num_elements->integer > 0xE0000000)) {
            uacpi_error("package is too large (%" UACPI_PRIu64 "), assuming "
    ...
    if (uacpi_unlikely(num_defined_elements > num_elements)) {
        uacpi_warn("too many package initializers: %u, truncating to %u\n",
    ...
    for (i = 0; i < num_defined_elements; ++i) {
        ...
        if (obj == UACPI_NULL) {
            ...
            ret = name_string_to_path(
                ctx->cur_frame,
                item_array_at(&op_ctx->items, base_pkg_index)->immediate, &path,
                &length);
            if (uacpi_unlikely_error(ret))
                return ret;
...
static uacpi_size sizeof_int(void) { return g_uacpi_rt_ctx.is_rev1 ? 4 : 8; }

static uacpi_status get_object_storage(uacpi_object *obj,
                                       uacpi_data_view *out_buf,
...
static void write_buffer_index(uacpi_buffer_index *buf_idx,
    ...
    uacpi_memcpy_zerout(buffer_index_cursor(buf_idx), src_buf->bytes, 1,
                        src_buf->length);
}
 ...
 */
static uacpi_status
object_assign_with_implicit_cast(uacpi_object *dst, uacpi_object *src,
    ...
    case UACPI_OBJECT_BUFFER: {
        ...
        uacpi_memcpy_zerout(dst_buf.bytes, src_buf.bytes, dst_buf.length,
                            src_buf.length);
        break;
    }

    case UACPI_OBJECT_BUFFER_FIELD:
        uacpi_write_buffer_field(&dst->buffer_field, src_buf.bytes,
                                 src_buf.length);
        break;

    case UACPI_OBJECT_FIELD_UNIT:
        return uacpi_write_field_unit(dst->field_unit, src_buf.bytes,
                                      src_buf.length, wtr_response);
...
out_bad_cast:
    uacpi_error("attempted to perform an invalid implicit cast (%s -> %s)\n",
...
static uacpi_status handle_arg_or_local(struct execution_context *ctx,
                                        uacpi_size idx,
                                        enum argx_or_localx type) {
    uacpi_object **src;
    struct item *dst;
...
static uacpi_status handle_create_op_region(struct execution_context *ctx) {
    ...
    if (op_region->space == UACPI_ADDRESS_SPACE_PCC &&
        op_region->offset > 255) {
        uacpi_warn("invalid PCC operation region %.4s subspace %" UACPI_PRIX64
                   "\n",
                   node->name.text, UACPI_FMT64(op_region->offset));
    }

    node->object =
        uacpi_create_internal_reference(UACPI_REFERENCE_KIND_NAMED, obj);
...
static uacpi_status table_id_error(const uacpi_char *opcode,
                                   const uacpi_char *arg, uacpi_buffer *str) {
    uacpi_error("%s: invalid %s '%s'\n", opcode, arg, str->text);
    return UACPI_STATUS_AML_BAD_ENCODING;
}

static void report_table_id_find_error(const uacpi_char *opcode,
                                       struct uacpi_table_identifiers *id,
                                       uacpi_status ret) {
    uacpi_error("%s: unable to find table '%.4s' (OEM ID '%.6s', "
                "OEM Table ID '%.8s'): %s\n",
                opcode, id->signature.text, id->oemid, id->oem_table_id,
                uacpi_status_to_string(ret));
}

static uacpi_status build_table_id(const uacpi_char *opcode,
                                   struct uacpi_table_identifiers *out_id,
                                   uacpi_buffer *signature,
                                   uacpi_buffer *oem_id,
    ...
    uacpi_memcpy_zerout(out_id->oemid, oem_id->text, sizeof(out_id->oemid),
                        oem_id->size ? oem_id->size - 1 : 0);
    ...
    uacpi_memcpy_zerout(out_id->oem_table_id, oem_table_id->text,
...
static uacpi_status handle_create_data_region(struct execution_context *ctx) {
    ...
    ret = build_table_id("DataTableRegion", &table_id,
...
static uacpi_status prepare_table_load(void *ptr,
    ...
    if (is_dynamic_table_load(cause)) {
        log_prefix = cause == UACPI_TABLE_LOAD_CAUSE_HOST
                         ? "host-invoked load of"
                         : "dynamic load of";
        log_level = UACPI_LOG_INFO;
    }

    uacpi_log_lvl(log_level, "%s " UACPI_PRI_TBL_HDR "\n", log_prefix,
                  UACPI_FMT_TBL_HDR(&dsdt->hdr));
...
static uacpi_status do_load_table(uacpi_namespace_node *parent,
                                  struct acpi_sdt_hdr *tbl,
                                  enum uacpi_table_load_cause cause) {
...
static uacpi_status handle_load_table(struct execution_context *ctx) {
    ...
    ret = build_table_id("LoadTable", &table_id,
    ...
    ret = prepare_table_load(table.hdr, UACPI_TABLE_LOAD_CAUSE_LOAD_TABLE_OP,
                             method);
    if (uacpi_unlikely_error(ret)) {
...
static uacpi_status handle_load(struct execution_context *ctx) {
    ...
    case UACPI_OBJECT_OPERATION_REGION: {
        ...
        if (uacpi_unlikely(op_region->space !=
        ...
        if (uacpi_unlikely(op_region->length < sizeof(struct acpi_sdt_hdr))) {
            uacpi_error("Load: operation region is too small: %" UACPI_PRIu64
                        "\n",
                        UACPI_FMT64(op_region->length));
        ...
        if (uacpi_unlikely(src_table == UACPI_NULL)) {
            uacpi_error("Load: failed to map operation region "
    ...
    case UACPI_OBJECT_BUFFER: {
        ...
        if (buffer->size < sizeof(struct acpi_sdt_hdr)) {
            uacpi_error("Load: buffer is too small: %zu\n", buffer->size);
    ...
    default:
        uacpi_error("Load: invalid argument '%s', expected "
...
static uacpi_status ensure_is_a_field_unit(uacpi_namespace_node *node,
    ...
    if (obj->type != UACPI_OBJECT_FIELD_UNIT) {
        uacpi_error("invalid argument: '%.4s' is not a field unit (%s)\n",
    ...
    while (i < item_array_size(&op_ctx->items)) {
        ...
        if (item->type == ITEM_NAMESPACE_NODE) {
            ...
            field->byte_offset = UACPI_ALIGN_DOWN(
                bit_offset / 8, field->access_width_bytes, uacpi_u32);
...
static uacpi_status begin_block_execution(struct execution_context *ctx) {
    ...
    cur_frame->last_while =
        find_last_block(&cur_frame->code_blocks, CODE_BLOCK_WHILE);
...
static void debug_store_no_recurse(const uacpi_char *prefix,
                                   uacpi_object *src) {
    switch (src->type) {
    ...
    case UACPI_OBJECT_INTEGER:
        if (g_uacpi_rt_ctx.is_rev1) {
            uacpi_trace("%s Integer => 0x%08X\n", prefix,
                        (uacpi_u32)src->integer);
        } else {
            uacpi_trace("%s Integer => 0x%016" UACPI_PRIX64 "\n", prefix,
    ...
    case UACPI_OBJECT_PACKAGE:
        uacpi_trace("%s Package @%p (%p) (%zu elements)\n", prefix, src,
                    src->package, src->package->count);
        break;
    case UACPI_OBJECT_BUFFER:
        uacpi_trace("%s Buffer @%p (%p) (%zu bytes)\n", prefix, src,
                    src->buffer, src->buffer->size);
        break;
    case UACPI_OBJECT_OPERATION_REGION:
        uacpi_trace(
    ...
    case UACPI_OBJECT_PROCESSOR:
        uacpi_trace("%s Processor[%d] 0x%08X (%d)\n", prefix,
                    src->processor->id, src->processor->block_address,
                    src->processor->block_length);
        break;
    case UACPI_OBJECT_BUFFER_INDEX:
        uacpi_trace("%s Buffer Index %p[%zu] => 0x%02X\n", prefix,
                    src->buffer_index.buffer->data, src->buffer_index.idx,
                    *buffer_index_cursor(&src->buffer_index));
        break;
    case UACPI_OBJECT_MUTEX:
        uacpi_trace("%s Mutex @%p (%p => %p) sync level %d\n", prefix, src,
                    src->mutex, src->mutex->handle, src->mutex->sync_level);
        break;
    ...
    default:
        uacpi_trace("%s %s @%p\n", prefix,
                    uacpi_object_type_to_string(src->type), src);
    }
}
...
opregion_try_detach_from_parent(void *user, uacpi_namespace_node *node,
...
static void object_replace_child(uacpi_object *parent,
                                 uacpi_object *new_child) {
...
static uacpi_status copy_object_to_reference(uacpi_object *dst,
    ...
    ret =
        uacpi_object_assign(new_obj, src_obj, UACPI_ASSIGN_BEHAVIOR_DEEP_COPY);
    if (uacpi_unlikely_error(ret))
        return ret;
 ...
 * 4. RefOf -> Not allowed here.
 */
static uacpi_status store_to_reference(uacpi_object *dst, uacpi_object *src,
                                       uacpi_data_view *wtr_response) {
...
static uacpi_status handle_ref_or_deref_of(struct execution_context *ctx) {
    ...
    if (op_ctx->op->code == UACPI_AML_OP_DerefOfOp) {
        ...
        if (src->type == UACPI_OBJECT_BUFFER_INDEX) {
            ...
            uacpi_memcpy_zerout(&dst->integer, buffer_index_cursor(buf_idx),
                                sizeof(dst->integer), 1);
            return UACPI_STATUS_OK;
        }

        if (!was_a_reference) {
            uacpi_error("invalid DerefOf argument: %s, expected a reference\n",
                        uacpi_object_type_to_string(src->type));
            return UACPI_STATUS_AML_INCOMPATIBLE_OBJECT_TYPE;
        }
...
static uacpi_status do_binary_math(uacpi_object *arg0, uacpi_object *arg1,
...
static uacpi_status ensure_valid_idx(uacpi_object *obj, uacpi_size idx,
    ...
    uacpi_error("invalid index %zu, %s@%p has %zu elements\n", idx,
                uacpi_object_type_to_string(obj->type), obj, src_size);
...
static uacpi_status handle_index(struct execution_context *ctx) {
    ...
    default:
        uacpi_error("invalid argument for Index: %s, "
...
static uacpi_u64 object_to_integer(const uacpi_object *obj,
    ...
    case UACPI_OBJECT_STRING:
        uacpi_string_to_integer(obj->buffer->text, obj->buffer->size,
                                UACPI_BASE_AUTO, &dst);
        break;
    default:
        dst = 0;
...
static uacpi_status integer_to_string(uacpi_u64 integer, uacpi_buffer *str,
                                      uacpi_bool is_hex) {
    int repr_len;
    uacpi_char int_buf[21];
    uacpi_size final_size;

    repr_len = uacpi_snprintf(int_buf, sizeof(int_buf),
...
static uacpi_status buffer_to_string(uacpi_buffer *buf, uacpi_buffer *str,
                                     uacpi_bool is_hex) {
    int repr_len;
    ...
    for (i = 0; i < buf->size; ++i) {
        repr_len =
            uacpi_snprintf(int_buf, sizeof(int_buf), is_hex ? "0x%02X" : "%d",
                           ((uacpi_u8 *)buf->data)[i]);
        if (uacpi_unlikely(repr_len < 0)) {
...
static uacpi_status handle_mid(struct execution_context *ctx) {
    ...
    if (uacpi_unlikely(src->type != UACPI_OBJECT_STRING &&
        ...
        uacpi_error("invalid argument for Mid: %s, expected String/Buffer\n",
                    uacpi_object_type_to_string(src->type));
        return UACPI_STATUS_AML_INCOMPATIBLE_OBJECT_TYPE;
    }
...
static uacpi_status handle_sizeof(struct execution_context *ctx) {
    ...
    default:
        uacpi_error("invalid argument for Sizeof: %s, "
                    "expected String/Buffer/Package\n",
                    uacpi_object_type_to_string(src->type));
        return UACPI_STATUS_AML_INCOMPATIBLE_OBJECT_TYPE;
    }
...
static uacpi_bool handle_logical_equality(uacpi_object *lhs,
                                          uacpi_object *rhs) {
    ...
    if (lhs->type == UACPI_OBJECT_STRING || lhs->type == UACPI_OBJECT_BUFFER) {
        ...
        if (res && lhs->buffer->size) {
            res = uacpi_memcmp(lhs->buffer->data, rhs->buffer->data,
                               lhs->buffer->size) == 0;
        }
...
static uacpi_bool handle_logical_less_or_greater(uacpi_aml_op op,
                                                 uacpi_object *lhs,
                                                 uacpi_object *rhs) {
    if (lhs->type == UACPI_OBJECT_STRING || lhs->type == UACPI_OBJECT_BUFFER) {
        int res;
...
static uacpi_status parse_package_length(struct call_frame *frame,
    ...
    if (uacpi_unlikely(out_pkg->end < out_pkg->begin)) {
        uacpi_error("PkgLength overflow: start=%u, size=%u\n", out_pkg->begin,
                    size);
        return UACPI_STATUS_AML_BAD_ENCODING;
    }
 ...
 */
static void init_method_flags(uacpi_control_method *method,
                              uacpi_u8 flags_byte) {
...
static uacpi_status handle_create_method(struct execution_context *ctx) {
    ...
    if (uacpi_unlikely(pkg->end < pkg->begin ||
        ...
        uacpi_error("invalid method %.4s bounds [%u..%u] (parent size is %u)\n",
                    node->name.text, method_begin_offset, pkg->end,
                    this_method->size);
        return UACPI_STATUS_AML_BAD_ENCODING;
    }
    ...
    node->object =
        uacpi_create_internal_reference(UACPI_REFERENCE_KIND_NAMED, dst);
    if (uacpi_unlikely(node->object == UACPI_NULL))
        return UACPI_STATUS_OUT_OF_MEMORY;
...
static uacpi_status
handle_create_mutex_or_event(struct execution_context *ctx) {
    struct op_context *op_ctx = ctx->cur_op_ctx;
    uacpi_namespace_node *node;
    uacpi_object *dst;
...
static uacpi_status handle_event_ctl(struct execution_context *ctx) {
    ...
    obj =
        uacpi_unwrap_internal_reference(item_array_at(&op_ctx->items, 0)->obj);
    if (uacpi_unlikely(obj->type != UACPI_OBJECT_EVENT)) {
        uacpi_error("%s: invalid argument '%s', expected an Event object\n",
...
static uacpi_status handle_mutex_ctl(struct execution_context *ctx) {
    ...
    if (uacpi_unlikely(obj->type != UACPI_OBJECT_MUTEX)) {
        uacpi_error("%s: invalid argument '%s', expected a Mutex object\n",
                    op_ctx->op->name, uacpi_object_type_to_string(obj->type));
        return UACPI_STATUS_AML_INCOMPATIBLE_OBJECT_TYPE;
    }
    ...
    case UACPI_AML_OP_AcquireOp: {
        ...
        if (uacpi_unlikely(ctx->sync_level > obj->mutex->sync_level)) {
            uacpi_warn(
                ...
                "(%d < %d)\n",
                obj->mutex, obj->mutex->sync_level, ctx->sync_level);
            break;
        }
    ...
    case UACPI_AML_OP_ReleaseOp: {
        ...
        ret = held_mutexes_array_remove_and_release(
            &ctx->held_mutexes, obj->mutex, FORCE_RELEASE_NO);
        if (uacpi_likely_success(ret)) {
...
static uacpi_status handle_notify(struct execution_context *ctx) {
    ...
    if (ret == UACPI_STATUS_NO_HANDLER) {
        ...
        uacpi_warn("ignoring firmware Notify(%s, 0x%" UACPI_PRIX64 ") request, "
                   "no listeners\n",
...
static uacpi_status handle_create_named(struct execution_context *ctx) {
    ...
    node->object =
        uacpi_create_internal_reference(UACPI_REFERENCE_KIND_NAMED, src);
    if (uacpi_unlikely(node->object == UACPI_NULL))
        return UACPI_STATUS_OUT_OF_MEMORY;
...
static uacpi_object_type
buffer_field_get_read_type(struct uacpi_buffer_field *field) {
...
static uacpi_status field_get_read_type(uacpi_object *obj,
...
static uacpi_status handle_field_read(struct execution_context *ctx) {
    ...
    ret = uacpi_read_field_unit(src_obj->field_unit, dst, dst_size,
                                &wtr_response);
    if (uacpi_unlikely_error(ret))
        return ret;
...
static uacpi_status handle_create_buffer_field(struct execution_context *ctx) {
    ...
    node->object =
        uacpi_create_internal_reference(UACPI_REFERENCE_KIND_NAMED, field_obj);
    if (uacpi_unlikely(node->object == UACPI_NULL))
        return UACPI_STATUS_OUT_OF_MEMORY;
...
static uacpi_status handle_control_flow(struct execution_context *ctx) {
    ...
    if (uacpi_unlikely(frame->last_while == UACPI_NULL)) {
        uacpi_error("attempting to %s outside of a While block\n",
                    op_ctx->op->code == UACPI_AML_OP_BreakOp ? "Break"
                                                             : "Continue");
        return UACPI_STATUS_AML_BAD_ENCODING;
    }
...
static uacpi_status handle_return(struct execution_context *ctx) {
     ...
     * be safe.
     */
    return uacpi_object_assign(dst,
...
static inline void trace_op(const struct uacpi_op_spec *op,
                            enum op_trace_action_type action) {
    uacpi_debug("%s OP '%s' (0x%04X)\n", op_trace_action_types[action],
                op->name, op->code);
}
...
static uacpi_status handle_inc_dec(struct execution_context *ctx) {
    ...
    case UACPI_OBJECT_BUFFER_FIELD:
        ...
        if (src->type == UACPI_OBJECT_FIELD_UNIT) {
            ret = uacpi_read_field_unit(src->field_unit, &dst->integer,
                                        sizeof_int(), UACPI_NULL);
            if (uacpi_unlikely_error(ret))
                return ret;
        } else {
...
static uacpi_status enter_method(struct execution_context *ctx,
    ...
    if (uacpi_unlikely(ctx->sync_level > method->sync_level)) {
        uacpi_error("cannot invoke method @%p, sync level %d is too low "
                    "(current is %d)\n",
...
static uacpi_status typecheck_obj(const struct op_context *op_ctx,
...
static uacpi_status typecheck_operand(const struct op_context *op_ctx,
                                      const uacpi_object *obj) {
    return typecheck_obj(op_ctx, obj, UACPI_OBJECT_INTEGER, SPEC_OPERAND);
}

static uacpi_status typecheck_string(const struct op_context *op_ctx,
                                     const uacpi_object *obj) {
    return typecheck_obj(op_ctx, obj, UACPI_OBJECT_STRING, SPEC_STRING);
}

static uacpi_status
typecheck_computational_data(const struct op_context *op_ctx,
                             const uacpi_object *obj) {
    switch (obj->type) {
    case UACPI_OBJECT_STRING:
    case UACPI_OBJECT_BUFFER:
...
static void trace_named_object_lookup_or_creation_failure(
    ...
    conv_ret = name_string_to_path(frame, offset, &requested_path, &length);
    ...
    if (requested_path && requested_path[0] != '\\') {
        prefix_path =
            uacpi_namespace_node_generate_absolute_path(frame->cur_scope);
    ...
    if (length == 5 && !is_create) {
        uacpi_log_lvl(level,
                      ...
                      "scope '%s': %s\n",
                      action, requested_path_to_print, prefix_path,
                      uacpi_status_to_string(ret));
    } else {
        uacpi_log_lvl(level, "unable to %s named object '%s%s%s': %s\n", action,
                      prefix_path, middle_part, requested_path_to_print,
                      uacpi_status_to_string(ret));
    }
...
static uacpi_status prepare_method_call(struct execution_context *ctx,
                                        uacpi_namespace_node *node,
                                        uacpi_control_method *method,
                                        enum method_call_type type,
...
static void apply_tracked_pkg(struct call_frame *frame,
                              struct op_context *op_ctx) {
    struct item *item;
...
static uacpi_status exec_op(struct execution_context *ctx) {
    ...
    for (;;) {
        ...
        case UACPI_PARSE_OP_LOAD_INLINE_IMM_AS_OBJECT: {
            ...
            uacpi_memcpy_zerout(dst, op_decode_cursor(op_ctx),
        ...
        case UACPI_PARSE_OP_IF_LAST_NOT_NULL: {
            ...
            if (op == UACPI_PARSE_OP_IF_LAST_NULL ||
                ...
                idx = op_decode_byte(op_ctx);
                is_null =
                    item_array_at(&op_ctx->items, idx)->handle == UACPI_NULL;
            }
        ...
        case UACPI_PARSE_OP_EXISTING_NAMESTRING_OR_NULL_IF_LOAD: {
            ...
            if (uacpi_unlikely_error(ret)) {
                ...
                if (frame->method->named_objects_persist &&
                    ...
                    abort_whileif =
                        first_ctx->op->code == UACPI_AML_OP_WhileOp ||
                ...
                trace_named_object_lookup_or_creation_failure(frame, offset, op,
                                                              trace_ret, lvl);
        ...
        case UACPI_PARSE_OP_STORE_TO_TARGET_INDIRECT: {
            ...
            if (op == UACPI_PARSE_OP_STORE_TO_TARGET_INDIRECT) {
                src =
                    item_array_at(&op_ctx->items, op_decode_byte(op_ctx))->obj;
            } else {
                src = item->obj;
            }
        ...
        case UACPI_PARSE_OP_DISPATCH_METHOD_CALL: {
            ...
            ret = prepare_method_call(ctx, node, method, METHOD_CALL_AML,
                                      UACPI_NULL);
            return ret;
        }
        ...
        case UACPI_PARSE_OP_DISPATCH_TABLE_LOAD: {
            ...
            ret = prepare_method_call(ctx, node, method, METHOD_CALL_TABLE_LOAD,
                                      UACPI_NULL);
            return ret;
        }
        ...
        case UACPI_PARSE_OP_CONVERT_NAMESTRING: {
            ...
            case UACPI_OBJECT_FIELD_UNIT: {
                ...
                if (uacpi_unlikely_error(ret)) {
                    ...
                    field_path =
                        uacpi_namespace_node_generate_absolute_path(item->node);

                    uacpi_error("unable to perform a read from field %s: "
...
static void ctx_reload_post_ret(struct execution_context *ctx) {
    ...
    if (method->is_serialized) {
        held_mutexes_array_remove_and_release(&ctx->held_mutexes, method->mutex,
                                              FORCE_RELEASE_YES);
...
static void trace_method_abort(struct code_block *block, uacpi_size depth) {
    ...
    if (block != UACPI_NULL && block->type == CODE_BLOCK_SCOPE) {
        absolute_path =
            uacpi_namespace_node_generate_absolute_path(block->node);
...
static void execution_context_release(struct execution_context *ctx) {
    ...
    while (held_mutexes_array_size(&ctx->held_mutexes) != 0) {
        held_mutexes_array_remove_and_release(
            &ctx->held_mutexes, *held_mutexes_array_last(&ctx->held_mutexes),
            FORCE_RELEASE_YES);
    }
