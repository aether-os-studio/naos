...
void uacpi_trace_region_error(uacpi_namespace_node *node, uacpi_char *message,
                              uacpi_status ret) {
    ...
    uacpi_error("%s (%s) operation region %s: %s\n", message, space_string,
                path, uacpi_status_to_string(ret));
    uacpi_free_dynamic_string(path);
}

static void trace_region_io(uacpi_field_unit *field, uacpi_address_space space,
                            uacpi_u64 offset, uacpi_region_op op,
    ...
    case UACPI_ADDRESS_SPACE_FFIXEDHW:
        uacpi_trace("write-then-read from [%s] %s[0x%016" UACPI_PRIX64 "] = "
                    "<buffer of %zu bytes>\n",
                    path, uacpi_address_space_to_string(space),
                    UACPI_FMT64(offset), data.buffer.length);
        break;
    case UACPI_ADDRESS_SPACE_SMBUS:
    case UACPI_ADDRESS_SPACE_GENERIC_SERIAL_BUS:
        uacpi_trace("%s [%s] %s[0x%016" UACPI_PRIX64 "] = "
                    "<buffer of %zu bytes>\n",
    ...
    case UACPI_ADDRESS_SPACE_GENERAL_PURPOSE_IO:
        uacpi_trace("%s [%s] %s pins[%u..%u] = 0x%" UACPI_PRIX64 "\n", type_str,
                    path, uacpi_address_space_to_string(space),
                    field->pin_offset,
                    (field->pin_offset + field->bit_length) - 1,
                    UACPI_FMT64(*data.integer));
        break;
    default:
        uacpi_trace("%s [%s] (%d bytes) %s[0x%016" UACPI_PRIX64
                    "] = 0x%" UACPI_PRIX64 "\n",
                    type_str, path, field->access_width_bytes,
                    uacpi_address_space_to_string(space), UACPI_FMT64(offset),
                    UACPI_FMT64(*data.integer));
        break;
    }
...
static uacpi_status region_run_reg(uacpi_namespace_node *node,
    ...
    reg_obj = uacpi_namespace_node_get_object_typed(reg_node,
    ...
    ret = uacpi_execute_control_method(reg_node, reg_obj->method, &method_args,
                                       UACPI_NULL);
    if (uacpi_unlikely_error(ret))
...
uacpi_node_get_address_space_handlers(uacpi_namespace_node *node) {
    uacpi_object *object;
...
static uacpi_address_space_handler *
find_handler(uacpi_address_space_handlers *handlers,
...
static uacpi_operation_region *
find_previous_region_link(uacpi_operation_region *region) {
...
out:
    if (region->state_flags & UACPI_OP_REGION_STATE_ATTACHED) {
        ...
        if (uacpi_unlikely_error(ret)) {
            uacpi_trace_region_error(node, "error during handler detach for",
                                     ret);
        }
    }
...
uacpi_bool
uacpi_address_space_handler_is_default(uacpi_address_space_handler *handler) {
...
do_install_or_uninstall_handler(uacpi_handle opaque, uacpi_namespace_node *node,
                                uacpi_u32 depth) {
    ...
    if (object->type == UACPI_OBJECT_OPERATION_REGION) {
        ...
        if (ctx->action == OPREGION_ITER_ACTION_INSTALL) {
            ...
            if (uacpi_unlikely(region->handler != ctx->handler)) {
                uacpi_trace_region_error(node, "handler mismatch for",
...
static uacpi_iteration_decision
do_run_reg(void *opaque, uacpi_namespace_node *node, uacpi_u32 depth) {
...
static uacpi_status
reg_or_unreg_all_opregions(uacpi_namespace_node *device_node,
                           enum uacpi_address_space space,
                           uacpi_u8 connection_code) {
    uacpi_address_space_handlers *handlers;
    ...
    uacpi_namespace_do_for_each_child(
        device_node, do_run_reg, UACPI_NULL, UACPI_OBJECT_OPERATION_REGION_BIT,
        UACPI_MAX_DEPTH_ANY, UACPI_SHOULD_LOCK_NO, perm_only, &ctx);

    uacpi_trace("%sactivated all '%s' opregions controlled by '%.4s', "
                "%zu _REG() calls (%zu errors)\n",
                connection_code == ACPI_REG_CONNECT ? "" : "de",
                uacpi_address_space_to_string(space), device_node->name.text,
                ctx.reg_executed, ctx.reg_errors);
    return UACPI_STATUS_OK;
}

static uacpi_address_space_handlers *
extract_handlers(uacpi_namespace_node *node) {
...
uacpi_status
uacpi_uninstall_address_space_handler(uacpi_namespace_node *device_node,
                                      enum uacpi_address_space space) {
    uacpi_status ret;
    uacpi_address_space_handlers *handlers;
...
uacpi_status uacpi_dispatch_opregion_io(uacpi_field_unit *field,
                                        uacpi_u32 offset, uacpi_region_op op,
                                        union uacpi_opregion_io_data data) {
    uacpi_status ret;
    uacpi_object *obj;
    uacpi_operation_region *region;
    ...
    if (uacpi_unlikely_error(ret)) {
        uacpi_trace_region_error(field->region, "unable to attach", ret);
        goto out;
    }
    ...
    if (uacpi_unlikely(is_oob)) {
        ...
        path = uacpi_namespace_node_generate_absolute_path(field->region);
        uacpi_error("out-of-bounds access to opregion %s[0x%" UACPI_PRIX64 "->"
                    "0x%" UACPI_PRIX64 "] at 0x%" UACPI_PRIX64
                    " (idx=%u, width=%d)\n",
    ...
    case UACPI_ADDRESS_SPACE_PCC: {
        ...
        if (op == UACPI_REGION_OP_READ) {
            uacpi_memcpy_zerout(data.integer, cursor, sizeof(*data.integer),
        ...
        if (offset >= 12 && offset < 16) {
            uacpi_memzero(&handler_data.pcc.buffer,
                          sizeof(handler_data.pcc.buffer));
    ...
    case UACPI_ADDRESS_SPACE_GENERAL_PURPOSE_IO:
        ...
        ret = uacpi_object_get_string_or_buffer(field->connection,
                                                &handler_data.gpio.connection);
        if (uacpi_unlikely_error(ret))
            goto io_done;

        op = op == UACPI_REGION_OP_READ ? UACPI_REGION_OP_GPIO_READ
    ...
    case UACPI_ADDRESS_SPACE_SMBUS:
        ...
        op = op == UACPI_REGION_OP_READ ? UACPI_REGION_OP_SERIAL_READ
