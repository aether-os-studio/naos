...
#define MAKE_PREDEFINED(c0, c1, c2, c3)                                        \
    {.name.text = {c0, c1, c2, c3}, .flags = UACPI_NAMESPACE_NODE_PREDEFINED}
...
static uacpi_object *
make_object_for_predefined(enum uacpi_predefined_namespace ns) {
    uacpi_object *obj;
...
uacpi_status uacpi_initialize_namespace(void) {
    ...
    for (ns = UACPI_PREDEFINED_NAMESPACE_GPE;
        ...
        uacpi_namespace_node_install(uacpi_namespace_root(),
                                     &predefined_namespaces[ns]);
    }
...
uacpi_namespace_get_predefined(enum uacpi_predefined_namespace ns) {
...
uacpi_status uacpi_namespace_node_uninstall(uacpi_namespace_node *node) {
    ...
    if (uacpi_unlikely(node->child != UACPI_NULL)) {
        uacpi_warn("refusing to uninstall node %.4s with a child (%.4s)\n",
...
uacpi_namespace_node *
uacpi_namespace_node_find_sub_node(uacpi_namespace_node *parent,
...
uacpi_status uacpi_namespace_node_find(uacpi_namespace_node *parent,
    ...
    return uacpi_namespace_node_resolve(parent, path, UACPI_SHOULD_LOCK_YES,
...
uacpi_status uacpi_namespace_node_resolve_from_aml_namepath(
    ...
    uacpi_namespace_node **out_node) {
    return uacpi_namespace_node_resolve(scope, path, UACPI_SHOULD_LOCK_YES,
                                        UACPI_MAY_SEARCH_ABOVE_PARENT_YES,
                                        UACPI_PERMANENT_ONLY_YES, out_node);
}

uacpi_object *
uacpi_namespace_node_get_object(const uacpi_namespace_node *node) {
...
uacpi_object *
uacpi_namespace_node_get_object_typed(const uacpi_namespace_node *node,
...
uacpi_status
uacpi_namespace_node_acquire_object_typed(const uacpi_namespace_node *node,
                                          uacpi_object_type_bits type_mask,
...
uacpi_status
uacpi_namespace_node_acquire_object(const uacpi_namespace_node *node,
                                    uacpi_object **out_obj) {
    return uacpi_namespace_node_acquire_object_typed(node, UACPI_OBJECT_ANY_BIT,
                                                     out_obj);
}
...
uacpi_status
uacpi_namespace_node_type_unlocked(const uacpi_namespace_node *node,
...
uacpi_status
uacpi_namespace_node_is_one_of_unlocked(const uacpi_namespace_node *node,
                                        uacpi_object_type_bits type_mask,
                                        uacpi_bool *out) {
    uacpi_object *obj;
...
uacpi_status uacpi_namespace_node_is(const uacpi_namespace_node *node,
                                     uacpi_object_type type, uacpi_bool *out) {
...
uacpi_status
uacpi_namespace_for_each_child_simple(uacpi_namespace_node *parent,
...
uacpi_status uacpi_namespace_node_next_typed(uacpi_namespace_node *parent,
    ...
    for (; node != UACPI_NULL; node = node->next) {
        ...
        ret = uacpi_namespace_node_is_one_of_unlocked(node, type_mask,
                                                      &is_one_of);
        if (uacpi_unlikely_error(ret))
            break;
        if (is_one_of)
...
uacpi_status uacpi_namespace_node_next(uacpi_namespace_node *parent,
                                       uacpi_namespace_node **iter) {
    return uacpi_namespace_node_next_typed(parent, iter, UACPI_OBJECT_ANY_BIT);
}
...
const uacpi_char *
uacpi_namespace_node_generate_absolute_path(const uacpi_namespace_node *node) {
