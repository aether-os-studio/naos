...
static uacpi_status map_one(const struct register_spec *spec,
                            struct register_mapping *mapping, uacpi_u8 idx) {
...
static uacpi_status get_reg(uacpi_u8 idx, const struct register_spec **out_spec,
...
static uacpi_status do_read_one(struct register_mapping *mapping, uacpi_u8 idx,
                                uacpi_u64 *out_value) {
...
static uacpi_status do_write_one(struct register_mapping *mapping, uacpi_u8 idx,
                                 uacpi_u64 in_value) {
    if (mapping->states[idx] != REGISTER_MAPPING_STATE_MAPPED)
        return UACPI_STATUS_OK;
...
uacpi_status uacpi_write_registers(enum uacpi_register reg_enum,
                                   uacpi_u64 in_value0, uacpi_u64 in_value1) {
    uacpi_status ret;
    const struct register_spec *reg;
    struct register_mapping *mapping;
