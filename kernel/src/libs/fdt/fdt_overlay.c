...
static int overlay_fixup_one_phandle(void *fdt, void *fdto, int symbols_off,
    ...
    return fdt_setprop_inplace_namelen_partial(fdto, fixup_off, name, name_len,
                                               poffset, &phandle_prop,
                                               sizeof(phandle_prop));
};
...
static int overlay_update_node_conflicting_references(void *fdto, int tree_node,
    ...
    fdt_for_each_subnode(fixup_child, fdto, fixup_node) {
        ...
        ret = overlay_update_node_conflicting_references(
            fdto, tree_child, fixup_child, fdt_phandle, fdto_phandle);
        if (ret)
            return ret;
    }
...
static int overlay_prevent_phandle_overwrite_node(void *fdt, int fdtnode,
    ...
    if (fdt_phandle && fdto_phandle) {
        ...
        ret = overlay_update_local_conflicting_references(fdto, fdt_phandle,
                                                          fdto_phandle);
        if (ret)
            return ret;
    }
    ...
    fdt_for_each_subnode(fdtochild, fdto, fdtonode) {
        ...
        ret = overlay_prevent_phandle_overwrite_node(fdt, fdtchild, fdto,
                                                     fdtochild);
        if (ret)
            return ret;
    }
...
static int overlay_prevent_phandle_overwrite(void *fdt, void *fdto) {
    ...
    fdt_for_each_subnode(fragment, fdto, 0) {
        ...
        ret =
            overlay_prevent_phandle_overwrite_node(fdt, target, fdto, overlay);
        if (ret)
            return ret;
    }
