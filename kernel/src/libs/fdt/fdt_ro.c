...
const char *fdt_get_string(const void *fdt, int stroffset, int *lenp) {
    ...
    if (fdt_magic(fdt) == FDT_MAGIC) {
        ...
        if ((stroffset >= 0) || (sw_stroffset > fdt_size_dt_strings(fdt)))
            goto fail;
        if (sw_stroffset < len)
            len = sw_stroffset;
    } else {
...
fdt_get_property_namelen_(const void *fdt, int offset, const char *name,
    ...
    for (offset = fdt_first_property_offset(fdt, offset); (offset >= 0);
        ...
        if (fdt_string_eq_(fdt, fdt32_ld_(&prop->nameoff), name, namelen)) {
            if (poffset)
                *poffset = offset;
            return prop;
        }
...
const void *fdt_getprop_by_offset(const void *fdt, int offset,
    ...
    if (namep) {
        ...
        if (!can_assume(VALID_INPUT)) {
            name = fdt_get_string(fdt, fdt32_ld_(&prop->nameoff), &namelen);
            *namep = name;
            if (!name) {
                if (lenp)
                    *lenp = namelen;
...
const char *fdt_get_symbol_namelen(const void *fdt, const char *name,
                                   int namelen) {
...
int fdt_stringlist_count(const void *fdt, int nodeoffset,
                         const char *property) {
    const char *list, *end;
    int length, count = 0;
...
const char *fdt_stringlist_get(const void *fdt, int nodeoffset,
                               const char *property, int idx, int *lenp) {
    const char *list, *end;
    int length;
