...
static inline void llist_append(struct llist_header *head,
                                struct llist_header *elem) {
    __llist_add(elem, head->prev, head);
}

static inline void llist_prepend(struct llist_header *head,
                                 struct llist_header *elem) {
    __llist_add(elem, head, head->next);
}

static inline void llist_insert_after(struct llist_header *head,
                                      struct llist_header *elem) {
    __llist_add(elem, head, head->next);
}
...
#define list_prev(current, type, member)                                       \
    container_of(current->member.prev, type, member)
...
static inline void hlist_add(struct hlist_node **head,
