 ...
 *
 * A Red-Black Tree (RBT) is a tree structure that maintains sorted data to
 * allow for efficient insertion, deletion, and lookup operations with a worst
 * case time complexity of `O(log n)`.
 *
 * ## Used As A Sorted Linked List
 *
 * The name "Red-Black Tree" can be a bit confusing. To the user of the tree, it
 * simply acts as a highly optimized sorted linked list.
 *
 * The tree structure allows for more efficient operations compared to a
 * standard linked list (`O(log n)` vs `O(n)`), and the red-black properties
 * ensure that the tree remains balanced, preventing it from degenerating into a
 * linear structure,
 *
 * However, the user of the tree does not need to be concerned with these
 * implementation details.
 *
 * TODO: Cache minimum and maximum nodes for `O(1)` access.
 *
 * @see [Wikipedia Red-Black
 * Tree](https://en.wikipedia.org/wiki/Red%E2%80%93black_tree) for more
 ...
 * @enum rbnode_color_t
 */
typedef enum { RBNODE_RED = 0, RBNODE_BLACK = 1 } rbnode_color_t;
...
#define RBNODE_FROM_PARENT(node)                                               \
    ((rbnode_direction_t)((node)->parent->children[RBNODE_RIGHT] == (node)     \
                              ? RBNODE_RIGHT                                   \
                              : RBNODE_LEFT))
 ...
 * @struct rbnode_t
 *
 * Should be embedded in the structure to be stored in the tree, such that the
 * parent structure can be retrieved via `CONTAINER_OF()`.
 */
...
#define RBNODE_CREATE                                                          \
    (rbnode_t) { .parent = NULL, .children = {NULL, NULL}, .color = RBNODE_RED }
 ...
 *
 * @see
 * https://upload.wikimedia.org/wikipedia/commons/f/f2/Binary_Tree_Rotation_%28animated%29.gif
 * for a animation of tree rotations.
 *
 ...
 */
rbnode_t *rbtree_rotate(rbtree_t *tree, rbnode_t *node,
 ...
 *
 * @param elem The loop variable, a pointer to the structure containing the tree
 * node.
...
#define RBTREE_FOR_EACH(elem, tree, member)                                    \
    for ((elem) = ((tree)->root != NULL                                        \
                       ? container_of(rbtree_find_min((tree)->root),           \
                                      typeof(*(elem)), member)                 \
                       : NULL);                                                \
         (elem) != NULL;                                                       \
         (elem) = (rbtree_next(&(elem)->member) != NULL                        \
                       ? container_of(rbtree_next(&(elem)->member),            \
                                      typeof(*(elem)), member)                 \
                       : NULL))
