...
static int random_level(void) {
    int level = 1;
    while ((nanoTime() & 0xFFFFFFFF) < UINT32_MAX / 2 && level < MAX_LEVEL) {
        level++;
    }
    return level;
}
...
struct sched_entity *sched_entity_create(task_t *task) {
    ...
    se->vruntime = 0;
    se->deadline = 0;
    se->time_slice = calc_timeslice(PRIO_TO_NICE(task->priority));
    se->weight = get_weight_for_nice(PRIO_TO_NICE(task->priority));
    se->is_yield = false;
    se->node = NULL;
    se->rq = NULL;
...
static struct skiplist_node *skiplist_create_node(int level,
...
/* Compare two scheduling entities
 * Returns: -1 if se1 < se2, 0 if equal, 1 if se1 > se2
 */
static inline int se_compare(struct sched_entity *se1,
                             struct sched_entity *se2) {
    if (!se1 || !se2)
        return 0;

    /* First compare by deadline */
    if (se1->deadline < se2->deadline)
        return -1;
    if (se1->deadline > se2->deadline)
        return 1;
...
static void skiplist_delete(struct rq *rq, struct sched_entity *se) {
    ...
    for (int i = 0; i < rq->max_level; i++) {
        if (update[i] && update[i]->forward[i] == x) {
...
static struct sched_entity *skiplist_peek_first(struct rq *rq) {
    ...
    while (node != NULL) {
        if (node->se->task->state == TASK_READY) {
            return node->se;
        }
        node = node->forward[0];
    }
...
static struct sched_entity *skiplist_find_next_ready(struct rq *rq,
                                                     task_t *curr) {
    struct skiplist_node *node = rq->head->forward[0];
    ...
    while (node != NULL) {
        ...
        if (se->task != curr && se->task->state == TASK_READY) {
            return se;
        }
        node = node->forward[0];
    }
...
static void update_min_vruntime(struct rq *rq) {
    ...
    /* Check current task */
    if (rq->curr) {
        struct sched_entity *se = (struct sched_entity *)rq->curr->sched_info;
        if (se) {
    ...
    if (leftmost) {
        if (!rq->curr) {
            ...
            struct sched_entity *curr_se =
                (struct sched_entity *)rq->curr->sched_info;
            min_vruntime = (curr_se->vruntime < leftmost->vruntime)
                               ? curr_se->vruntime
                               : leftmost->vruntime;
        }
    }
...
void enqueue_task(struct rq *rq, task_t *task) {
...
void dequeue_task(struct rq *rq, task_t *task) {
    struct sched_entity *se = (struct sched_entity *)task->sched_info;
    if (!se || !se->node || !se->rq)
        return;
...
task_t *pick_next_task(struct rq *rq) {
    ...
    se->time_slice = calc_timeslice(PRIO_TO_NICE(se->task->priority));
...
static void handle_yield(struct rq *rq) {
    ...
    task_t *yielding_task = rq->curr;
    struct sched_entity *se = (struct sched_entity *)yielding_task->sched_info;
    ...
    if (yielding_task->state == TASK_READY) {
    ...
    struct sched_entity *next_se_final =
        (struct sched_entity *)next->sched_info;
    if (next_se_final) {
        next_se_final->exec_start = rq->clock;
    }
}
...
void schedule(struct rq *rq) {
    ...
    struct sched_entity *curr_se = (struct sched_entity *)rq->curr->sched_info;
    ...
    } else if (curr_se->time_slice == 0 || rq->curr->state != TASK_READY) {
        need_resched = true;

        /* Dequeue if not ready */
        if (rq->curr->state != TASK_READY) {
