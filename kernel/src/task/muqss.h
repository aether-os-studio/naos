...
#define CLAMP_NICE(nice)                                                       \
    ((nice) < MIN_NICE ? MIN_NICE : ((nice) > MAX_NICE ? MAX_NICE : (nice)))

/* Clamp priority to valid range */
#define CLAMP_PRIO(prio)                                                       \
    ((prio) < 0 ? 0 : ((prio) >= MAX_PRIO ? (MAX_PRIO - 1) : (prio)))
...
#define PRIO_TO_WEIGHT(prio)                                                   \
    (sched_prio_to_weight[PRIO_TO_NICE(prio) - MIN_NICE])
...
#define PRIO_TO_TIMESLICE(prio)                                                \
    (MIN_TIMESLICE + ((MAX_PRIO - 1 - (prio)) *                                \
                      (MAX_TIMESLICE - MIN_TIMESLICE) / (MAX_PRIO - 1)))
...
#define NICE_DELTA(delta_exec, nice)                                           \
    CALC_DELTA(delta_exec, NICE_TO_WEIGHT(nice))

#define PRIO_DELTA(delta_exec, prio)                                           \
    CALC_DELTA(delta_exec, PRIO_TO_WEIGHT(prio))
...
struct sched_entity {
    task_t *task;              /* Back pointer to task */
...
extern struct rq schedulers[MAX_CPU_NUM];
