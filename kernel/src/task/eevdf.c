...
static inline int64_t entity_key(eevdf_t *eevdf_sched, struct sched_entity *entity)
{
...
struct sched_entity *new_entity(task_t *task, uint64_t prio, eevdf_t *eevdf_sched)
{
...
struct sched_entity *pick_eevdf(eevdf_t *eevdf_sched)
{
    ...
    while (node)
    {
        ...
        if (left &&
            vruntime_eligible(eevdf_sched, container_of(left, struct sched_entity, run_node)->min_vruntime))
        {
            node = left;
            continue;
        }
...
static void update_min_vruntime(eevdf_t *eevdf_sched)
{
    ...
    eevdf_sched->min_vruntime = MAX(__update_min_vruntime(eevdf_sched, vruntime), vruntime);
}
...
void remove_sched_entity(eevdf_t *eevdf_sched, struct rb_root *root, struct sched_entity *se)
{
