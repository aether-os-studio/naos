...
void procfs_task_mem_stats(task_t *task, procfs_task_mem_stats_t *stats) {
    ...
    while (node) {
        ...
        if ((vma->vm_flags & VMA_SHARED) || vma->vm_type == VMA_TYPE_SHM)
    ...
    uint64_t remaining = stats->resident_pages > stats->shared_pages
                             ? stats->resident_pages - stats->shared_pages
                             : 0;
    stats->file_pages = MIN(file_size_pages, remaining);
    remaining =
        remaining > stats->file_pages ? remaining - stats->file_pages : 0;
