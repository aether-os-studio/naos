...
char *meminfo_origin[] = {"MemTotal:\t\t%llu kB\n",
                          ...
                          "DirectMap1G:\t%llu kB\n",
                          "PageCacheLru:\t%llu kB\n",
                          "PageCacheMapped:\t%llu kB\n",
                          "PageCacheReclaimable:\t%llu kB\n",
                          "PageCacheReclaimed:\t%llu kB\n",
                          "PageCacheReclaimScanned:\t%llu kB\n"};

char *proc_gen_meminfo(size_t *context_len) {
    const size_t field_count =
        sizeof(meminfo_origin) / sizeof(meminfo_origin[0]);
    uint64_t values[sizeof(meminfo_origin) / sizeof(meminfo_origin[0])] = {0};
    ...
    uint64_t mapped_cache_pages = MIN(cache.mapped_pages, cache.cached_pages);
    uint64_t inactive_file_pages = cache.cached_pages > mapped_cache_pages
                                       ? cache.cached_pages - mapped_cache_pages
                                       : 0;
    uint64_t clean_cache_pages = cache.cached_pages > cache.dirty_pages
                                     ? cache.cached_pages - cache.dirty_pages
                                     : 0;
    uint64_t reclaimable_cache_pages =
        clean_cache_pages > mapped_cache_pages
            ? clean_cache_pages - mapped_cache_pages
            : 0;
    ...
    values[2] =
        (free_pages + reclaimable_pages + reclaimable_cache_pages) * page_kb;
    values[3] = 0 * page_kb;
    values[4] = cache.cached_pages * page_kb;
    values[6] = mapped_cache_pages * page_kb;
    values[7] = inactive_file_pages * page_kb;
    values[10] = mapped_cache_pages * page_kb;
    values[11] = inactive_file_pages * page_kb;
    values[18] = cache.dirty_pages * page_kb;
    values[19] = cache.writeback_pages * page_kb;
    values[21] = mapped_cache_pages * page_kb;
    values[23] = reclaimable_pages * page_kb;
    values[25] = reclaimable_pages * page_kb;
    values[33] = values[0];
    values[51] = 2048;
    values[53] = values[0];
    values[55] = cache.lru_pages * page_kb;
    values[56] = cache.mapped_pages * page_kb;
    values[57] = reclaimable_cache_pages * page_kb;
    values[58] = cache.reclaimed_pages * page_kb;
    values[59] = cache.reclaim_scanned_pages * page_kb;
