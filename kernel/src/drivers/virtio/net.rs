...
#[derive(Clone)]
pub struct VirtIONetDriver(Arc<VirtIONet<HalImpl, PciTransport, 2>>);
...
impl phy::Device for VirtIONetDriver {
    ...
    fn capabilities(&self) -> DeviceCapabilities {
        ...
        caps.max_transmission_unit = 4096;
...
static mut ACTIVATE_DRIVER: Option<Arc<VirtIONetInterface>> = None;

static VIRTIO_NET_DRIVER: Mutex<Vec<Arc<VirtIONetInterface>>> = Mutex::new(Vec::new());

pub fn init_pci(transport: PciTransport) {
    if let Ok(net) = VirtIONet::new(transport, 4096) {
        let mut net_driver = VirtIONetDriver(Arc::new(net));

        let config = Config::new(smoltcp::wire::HardwareAddress::Ethernet(
            EthernetAddress::from_bytes(&net_driver.0.mac_address()),
        ));
        ...
        VIRTIO_NET_DRIVER.lock().push(interface.clone());

        unsafe { ACTIVATE_DRIVER = Some(interface) };
    }
}
