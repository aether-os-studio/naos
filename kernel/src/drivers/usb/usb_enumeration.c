// usb_enumeration.c
#include <drivers/usb/usb_enumeration.h>
#include <mm/mm.h>
...
// 创建枚举上下文
static usb_enum_context_t *usb_alloc_enum_context(usb_hcd_t *hcd,
                                                  uint8_t port_id) {
    usb_enum_context_t *ctx =
        (usb_enum_context_t *)malloc(sizeof(usb_enum_context_t));
    if (!ctx) {
        return NULL;
    }
...
// 解析配置描述符
int usb_parse_config_descriptor(usb_device_t *device, uint8_t *buffer,
                                uint32_t length) {
    ...
    device->config_descriptor =
        (usb_config_descriptor_t *)malloc(config->wTotalLength);
    if (!device->config_descriptor) {
        return -1;
    }
    ...
    while (ptr < end) {
        ...
        case USB_DT_ENDPOINT: {
            ...
            if (endpoint_index < 32) {
                ...
                device->endpoints[endpoint_index].max_packet_size =
                    ep->wMaxPacketSize;
        ...
        default:
            printk("  Unknown descriptor type: 0x%02x\n", desc_type);
            break;
        }
...
int usb_configure_device_endpoints(usb_device_t *device) {
    if (!device || !device->hcd || !device->hcd->ops->configure_endpoint) {
        return -1;
    }
...
int usb_enum_state_machine(usb_enum_context_t *ctx) {
    ...
    case USB_ENUM_STATE_GET_DESC_8:
        ...
        if (desc->bMaxPacketSize0 !=
            ctx->device->endpoints[0].max_packet_size) {
    ...
    case USB_ENUM_STATE_GET_DEVICE_DESC:
        ...
        // 保存设备描述符
        memcpy(&ctx->device->descriptor, ctx->buffer,
               sizeof(usb_device_descriptor_t));
    ...
    case USB_ENUM_STATE_ERROR:
        ...
        if (ctx->device) {
            ...
            if (ctx->hcd->ops->disable_slot) {
                ctx->hcd->ops->disable_slot(ctx->hcd, ctx->device);
            }
            usb_free_device(ctx->device);
            ctx->device = NULL;
        }
