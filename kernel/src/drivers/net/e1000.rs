...
use crate::net::net_core::{SOCKETS, SOCKETS_SET};
...
static E1000_DRIVER: Lazy<Mutex<Vec<Arc<E1000Interface>>>> = Lazy::new(|| {
    ...
    for i in 0..num {
        ...
        if device.vendor_id != 0x8086 || (device.device_id != 0x100e && device.device_id != 0x100f)
        {
            continue;
        }
...
unsafe extern "C" fn e1000_init() {
    unsafe {
        task_create("e1000_net\0".as_ptr() as _, Some(e1000_init_thread), 0);
    }
}
