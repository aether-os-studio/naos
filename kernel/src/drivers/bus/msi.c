...
__msi_read_cap_list(struct msi_desc_t *msi_desc, uint32_t cap_off) {
    ...
    cap_list.msg_addr_lo = ptr->op->read(ptr->bus, ptr->slot, ptr->func,
                                         ptr->segment, cap_off + 0x4);
    uint16_t msg_data_off = 0xc;
    if (cap_list.msg_ctrl & (1 << 7)) // 64位
    {
        cap_list.msg_addr_hi = ptr->op->read(ptr->bus, ptr->slot, ptr->func,
    ...
    cap_list.msg_data = ptr->op->read(ptr->bus, ptr->slot, ptr->func,
                                      ptr->segment, cap_off + msg_data_off) &
                        0xffff;

    cap_list.mask = ptr->op->read(ptr->bus, ptr->slot, ptr->func, ptr->segment,
                                  cap_off + 0x10);
    cap_list.pending = ptr->op->read(ptr->bus, ptr->slot, ptr->func,
...
__msi_read_msix_cap_list(struct msi_desc_t *msi_desc, uint32_t cap_off) {
    ...
    cap_list.dword1 = ptr->op->read(ptr->bus, ptr->slot, ptr->func,
                                    ptr->segment, cap_off + 0x4);
    cap_list.dword2 = ptr->op->read(ptr->bus, ptr->slot, ptr->func,
                                    ptr->segment, cap_off + 0x8);
    return cap_list;
}
