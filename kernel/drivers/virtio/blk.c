...
int virtio_blk_init(virtio_driver_t *driver)
{
    ...
    for (uint32_t i = 0; i < sizeof(virtio_blk_config_t) / sizeof(uint32_t); i++)
    {
        uint32_t value = driver->op->read_config_space(driver->data, i * sizeof(uint32_t));
        memcpy((uint8_t *)&config + i * sizeof(uint32_t), &value, sizeof(uint32_t));
    }

    // Create request queue
    virtqueue_t *request_queue = virt_queue_new(driver, 0, !!(features & (1 << 28)), !!(features & (1 << 29)));
    if (!request_queue)
    {
    ...
    virtio_blk_device_t *blk_device = (virtio_blk_device_t *)malloc(sizeof(virtio_blk_device_t));
    memset(blk_device, 0, sizeof(virtio_blk_device_t));

    blk_device->driver = driver;
    blk_device->capacity = config.capacity;
    blk_device->block_size = config.blk_size ? config.blk_size : 512; // Default to 512 bytes
    blk_device->request_queue = request_queue;
    blk_device->sector_size = 512; // Standard sector size

    printk("virtio_blk: Found block device with capacity %llu sectors (%llu MB), block size: %u bytes\n",
           config.capacity, (config.capacity * blk_device->sector_size) / (1024 * 1024), blk_device->block_size);
    ...
    regist_blkdev((char *)"virtio-blk", blk_device, blk_device->block_size, config.capacity * blk_device->sector_size, DEFAULT_PAGE_SIZE * 32, virtio_read, virtio_write);
...
int virtio_blk_read(virtio_blk_device_t *blk_dev, uint64_t sector, void *buffer, uint32_t count)
{
    ...
    virtio_blk_req_t *req_header = (virtio_blk_req_t *)alloc_frames_bytes(sizeof(virtio_blk_req_t));
    ...
    *status_byte = 0xFF; // Initialize to invalid status
    ...
    uint16_t desc_idx = virt_queue_add_buf(blk_dev->request_queue, bufs, 3, writable);
    if (desc_idx == 0xFFFF)
    {
    ...
    while ((used_desc_idx = virt_queue_get_used_buf(blk_dev->request_queue, &len)) == 0xFFFF)
    {
...
int virtio_blk_flush(virtio_blk_device_t *blk_dev)
{
    ...
    *status_byte = 0xFF; // Initialize to invalid status
    ...
    uint16_t desc_idx = virt_queue_add_buf(blk_dev->request_queue, bufs, 2, writable);
    if (desc_idx == 0xFFFF)
    {
        free_frames_bytes(req_header, sizeof(virtio_blk_req_t));
        free_frames_bytes(status_byte, sizeof(uint8_t));
