...
NVME_CONTROLLER *nvme_driver_init(uint64_t bar0, uint64_t bar_size)
{
    ...
    map_page_range(get_current_page_dir(false), (uint64_t)cap, bar0, bar_size, PT_FLAG_R | PT_FLAG_W | PT_FLAG_UNCACHEABLE);
    ...
    sqe.META = 0;
    sqe.DATA[0] = translate_address(get_current_page_dir(false), (uint64_t)identify);
    sqe.DATA[1] = 0;
    sqe.NSID = 0;
    ...
    sqe.CDW0 = NVME_SQE_OPC_ADMIN_IDENTIFY;
    sqe.META = 0;
    sqe.DATA[0] = translate_address(get_current_page_dir(false), (uint64_t)identifyNS);
    sqe.DATA[1] = 0;
    sqe.NSID = nsid;
