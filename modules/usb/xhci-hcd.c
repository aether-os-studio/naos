...
static void xhci_process_events(struct usb_xhci_s *xhci) {
    ...
    for (;;) {
        ...
        uint32_t trb_c = !!(etrb->control & TRB_C);
        if (trb_c != cs) {
            break; // 没有更多事件
        }

        uint32_t evt_type = TRB_TYPE(etrb->control);
        uint32_t evt_cc = (etrb->status >> 24) & 0xff;

        switch (evt_type) {
        case ER_TRANSFER: {
            uint32_t slotid = xhci_get_field(etrb->control, TRB_CR_SLOTID);
            uint32_t epid = xhci_get_field(etrb->control, TRB_CR_EPID);
...
static int xhci_event_wait(struct usb_xhci_s *xhci, struct xhci_ring *ring,
    ...
    for (;;) {
        ...
        if (nanoTime() > timeout_ns) {
            ...
            printk("ring->eidx = %d\n", ring->eidx);
            printk("ring->nidx = %d\n", ring->nidx);
            timeout_ns = nanoTime() + (uint64_t)timeout * 1000000ULL;
            continue;
        }
        arch_pause();
    }
