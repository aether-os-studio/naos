...
int cpfs_mount(vfs_node_t han, vfs_node_t node) {
    //TODO if (handle != (void *)CPFS_REGISTER_ID) return -ENODEV;
    node->fsid        = cpfs_id;
    cpfs_root_vfs     = node;
...
ssize_t cpfs_read(fd_t *file, void *addr, size_t offset, size_t size) {
    cpfs_file_t *f = (cpfs_file_t *)file->node->handle;
...
ssize_t cpfs_write(fd_t *file, const void *addr, size_t offset, size_t size) {
    cpfs_file_t *f   = (cpfs_file_t *)file->node->handle;
...
int cpfs_mk(void *parent, const char *name, vfs_node_t node, bool is_dir) {
    cpfs_file_t *p = (cpfs_file_t *)parent;
...
int cpfs_mkdir(void *parent, const char *name, vfs_node_t node) {
    return cpfs_mk(parent, name, node, true);
}

int cpfs_mkfile(void *parent, const char *name, vfs_node_t node) {
    return cpfs_mk(parent, name, node, false);
}

int cpfs_stat(void *file, vfs_node_t node) {
    cpfs_file_t *file0 = (cpfs_file_t *)file;
    if (file0 == NULL) return -ENODEV;
    node->type = file0->is_symlink ? file_symlink : (file0->is_dir ? file_dir : file_none);
    node->size = file0->is_dir ? 0 : file0->size;
    return EOK;
}

int cpfs_delete(void *parent, vfs_node_t node) {
    cpfs_file_t *p = (cpfs_file_t *)parent;
...
int cpfs_rename(void *current, const char *new_name) {
...
void *cpfs_map(void *file, void *addr, size_t offset, size_t size, size_t prot, size_t flags) {
   //TODO return general_map(cpfs_read, file, (uint64_t)addr, offset, size, prot, flags);
   return NULL;
}

int cpfs_symlink(void *parent, const char *name, vfs_node_t node) {
...
void cpfs_unmount(vfs_node_t root) {
    cpfs_file_t *handle = root->handle;
...
static struct vfs_callback cpfs_callbacks = {
    ...
    .stat     = cpfs_stat,
    .open     = cpfs_open,
    .read     = (vfs_read_t)cpfs_read,
    .write    = (vfs_write_t)cpfs_write,
    ...
    .rename   = cpfs_rename,
    .poll     = cpfs_poll,
    .map      = (vfs_mapfile_t)cpfs_map,
};

void cpfs_setup() {
    //TODO需要naos方式的注册 magic为最后一个参数
