...
static inline void
ext4_extent_header_set_magic(struct ext4_extent_header *header,
                             uint16_t magic) {
 ...
 * breaking it into parts
 */
static void ext4_idx_store_pblock(struct ext4_extent_index *ix,
                                  ext4_fsblk_t pb) {
 ...
 * the header must be checked before calling this
 */
static void ext4_ext_binsearch(struct ext4_extent_path *path,
                               ext4_lblk_t block) {
    struct ext4_extent_header *eh = path->header;
...
static int ext4_find_extent(struct ext4_inode_ref *inode_ref, ext4_lblk_t block,
                            struct ext4_extent_path **orig_path,
                            uint32_t flags) {
    struct ext4_extent_header *eh;
    ...
    if (!path) {
        ...
        path =
            ext4_calloc(1, sizeof(struct ext4_extent_path) * (path_depth + 1));
        if (!path)
            return ENOMEM;
        ...
        if (i == depth) {
            ...
            if (m) {
                ...
                path[i].header->entries_count =
                    to_le16(to_le16(path[i].header->entries_count) - m);
...
static int ext4_ext_grow_indepth(struct ext4_inode_ref *inode_ref,
    ...
    if (ext_depth(inode_ref->inode))
        goal =
            ext4_idx_pblock(EXT_FIRST_INDEX(ext_inode_hdr(inode_ref->inode)));
    else
...
again:
    ...
    if (ret == EIO && need_split == true) {
        ...
        if (level > 0) {
            ...
            ret = ext4_ext_split_node(inode_ref, path, i, newext, npath,
                                      &ins_right_leaf);
            if (ret != EOK)
                goto out;
            ...
            while (--level >= 0) {
                if (ins_right_leaf)
                    ext4_ext_replace_path(inode_ref, path, &npath[level],
                                          i + level);
...
ext4_ext_next_allocated_block(struct ext4_extent_path *path) {
    ...
    while (depth >= 0) {
        if (depth == path->depth) {
            /* leaf */
            if (path[depth].extent &&
                path[depth].extent != EXT_LAST_EXTENT(path[depth].header))
...
__unused static void print_path(struct ext4_extent_path *path) {
    ...
    while (i >= 0) {
        ...
        ptrdiff_t a = (path->extent)
                          ? (path->extent - EXT_FIRST_EXTENT(path->header))
                          : 0;
        ptrdiff_t b =
            (path->index) ? (path->index - EXT_FIRST_INDEX(path->header)) : 0;
...
int ext4_extent_get_blocks(struct ext4_inode_ref *inode_ref, ext4_lblk_t iblock,
    ...
    if (ex) {
        ...
        if (IN_RANGE(iblock, ee_block, ee_len)) {
            ...
            newblock = iblock - ee_block + ee_start;
            err =
                ext4_ext_zero_unwritten_range(inode_ref, newblock, zero_range);
            if (err != EOK)
                goto out2;
