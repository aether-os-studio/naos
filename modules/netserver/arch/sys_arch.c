...
void sys_mutex_lock(spinlock_t *spinlock) { spin_lock(spinlock); }
void sys_mutex_unlock(spinlock_t *spinlock) { spin_unlock(spinlock); }

err_t sys_mutex_new(spinlock_t *spinlock) {
...
sys_thread_t sys_thread_new(const char *pcName,
    ...
    task_t *task = task_create(pcName, (void (*)(uint64_t))pxThread,
                               (uint64_t)pvArg, KTHREAD_PRIORITY);
    return task->pid;
}
...
// mmm, trash
void *sio_open(u8_t devnum) { return 0; }
u32_t sio_write(void *fd, const u8_t *data, u32_t len) { return 0; }
void sio_send(u8_t c, void *fd) {}
u8_t sio_recv(void *fd) { return 0; }
u32_t sio_read(void *fd, u8_t *data, u32_t len) { return 0; }
u32_t sio_tryread(void *fd, u8_t *data, u32_t len) { return 0; }
