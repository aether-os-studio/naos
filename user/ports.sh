...
sudo chroot "$SYSROOT/" /bin/bash --login -c "apk add musl-dev seatd eudev gcompat gzip xz make file tar pciutils tzdata nano vim lua5.1 gcc binutils libdrm-dev libdrm-tests bind-tools curl sysbench evtest"
sudo chroot "$SYSROOT/" /bin/bash --login -c "apk add weston weston-clients weston-backend-drm weston-xwayland xwayland weston-shell-desktop weston-terminal ttf-dejavu gnome-mines"
# sudo chroot "$SYSROOT/" /bin/bash --login -c "apk add xorg-server xf86-video-fbdev xf86-input-evdev xinit twm xsetroot nedit st"
sudo chroot "$SYSROOT/" /bin/bash --login -c "apk add xeyes xclock mesa-gl mesa-utils mesa-vulkan-swrast mesa-dri-gallium"
